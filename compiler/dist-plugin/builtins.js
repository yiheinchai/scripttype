...
exports.isPredicate = exports.BUILTINS = void 0;
exports.guardEquivalent = guardEquivalent;
/**
 ...
 */
function fnNode(params, ret, isCtor) {
    if (params.kind !== 'tuple')
        throw new Error('expected a parameter tuple');
    const optionalAt = [];
    let hasRest = false;
    params.elements.forEach((el, i) => {
        if (el.optional)
            optionalAt.push(i);
        if (el.spread && i === params.elements.length - 1)
            hasRest = true;
    });
    return {
        kind: 'fn',
...
register(def({
    ...
    lower: ([s, p]) => ({
        tag: 'match',
        check: s,
        ext: (0, ir_js_1.template)(['', '', ''], [p, ANY_STR]),
        binds: [],
    }),
}), def({
    name: 'endsWith',
    ...
    lower: ([s, p]) => ({
        tag: 'match',
        check: s,
        ext: (0, ir_js_1.template)(['', '', ''], [ANY_STR, p]),
        binds: [],
    }),
}), def({
    name: 'includes',
    ...
    lower: ([s, p]) => ({
        tag: 'match',
        check: s,
        ext: (0, ir_js_1.template)(['', '', '', ''], [ANY_STR, p, ANY_STR]),
        binds: [],
    }),
}), def({
    name: 'splitOnce',
    ...
    lower: ([s, sep], ctx) => {
        ...
        return {
            tag: 'match',
            check: s,
            ext: (0, ir_js_1.template)(['', '', '', ''], [(0, ir_js_1.infer)(a), sep, (0, ir_js_1.infer)(b)]),
            binds: [a, b],
        };
    },
}), def({
    ...
    lower: ([s, p], ctx) => {
        ...
        return expr((0, ir_js_1.cond)(s, (0, ir_js_1.template)(['', '', ''], [p, (0, ir_js_1.infer)(r)]), (0, ir_js_1.ref)(r), s));
    },
}), def({
    name: 'removeSuffix',
    arity: 2,
    ...
    lower: ([s, p], ctx) => {
        const r = ctx.fresh('Rest');
        return expr((0, ir_js_1.cond)(s, (0, ir_js_1.template)(['', '', ''], [(0, ir_js_1.infer)(r), p]), (0, ir_js_1.ref)(r), s));
    },
}), def({
    name: 'upper',
    arity: 1,
    doc: 'upper(s) -> Uppercase<S>',
    lower: ([s]) => expr((0, ir_js_1.ref)('Uppercase', [s])),
}), def({
    name: 'lower',
    arity: 1,
    doc: 'lower(s) -> Lowercase<S>',
    lower: ([s]) => expr((0, ir_js_1.ref)('Lowercase', [s])),
}), def({
    name: 'capitalize',
    arity: 1,
    doc: 'capitalize(s) -> Capitalize<S>',
    lower: ([s]) => expr((0, ir_js_1.ref)('Capitalize', [s])),
}), def({
    name: 'uncapitalize',
    arity: 1,
    doc: 'uncapitalize(s) -> Uncapitalize<S>',
    lower: ([s]) => expr((0, ir_js_1.ref)('Uncapitalize', [s])),
}), def({
    name: 'concatStr',
    arity: 2,
    doc: 'concatStr(a, b) -> `${A}${B}`',
    lower: ([a, b]) => expr((0, ir_js_1.template)(['', '', ''], [a, b])),
}), def({
    name: 'trim',
    arity: 1,
    ...
    lower: ([s], ctx) => {
        ctx.usePrelude('Trim');
        return expr((0, ir_js_1.ref)('Trim', [s]));
    },
}), def({
    name: 'splitLast',
    arity: 2,
    ...
    lower: ([s, sep], ctx) => {
        ...
        return expr((0, ir_js_1.ref)('SplitLast', [s, sep]));
    },
}), def({
    name: 'split',
    arity: 2,
    ...
    lower: ([s, sep], ctx) => {
        ...
        return expr((0, ir_js_1.ref)('Split', [s, sep]));
    },
}), def({
    name: 'replaceAll',
    arity: 3,
    ...
    lower: ([s, f, t], ctx) => {
        ...
        return expr((0, ir_js_1.ref)('ReplaceAll', [s, f, t]));
    },
}), def({
    name: 'strLength',
    arity: 1,
    ...
    lower: ([s], ctx) => {
        ...
        return expr((0, ir_js_1.ref)('StrLength', [s]));
    },
}));
// ---------------------------------------------------------------------------
// Tuple / array
...
register(def({
    ...
    arity: 2,
    doc: 'concat(a, b) -> [...A, ...B]',
    lower: ([a, b]) => expr((0, ir_js_1.tuple)([{ expr: a, spread: true }, { expr: b, spread: true }])),
}), def({
    name: 'append',
    arity: 2,
    doc: 'append(t, x) -> [...T, X]',
    lower: ([t, x]) => expr((0, ir_js_1.tuple)([{ expr: t, spread: true }, { expr: x }])),
}), def({
    name: 'prepend',
    arity: 2,
    doc: 'prepend(t, x) -> [X, ...T]',
    lower: ([t, x]) => expr((0, ir_js_1.tuple)([{ expr: x }, { expr: t, spread: true }])),
}), def({
    name: 'length',
    arity: 1,
    doc: "length(t) -> T['length']",
    lower: ([t]) => expr((0, ir_js_1.indexed)(t, (0, ir_js_1.str)('length'))),
}), def({
    name: 'at',
    arity: 2,
    doc: 'at(t, i) -> T[I]',
    lower: ([t, i]) => expr((0, ir_js_1.indexed)(t, i)),
}), def({
    name: 'isEmpty',
    arity: 1,
    doc: 'isEmpty(t) -> T extends []',
    lower: ([t]) => ({ tag: 'match', check: t, ext: (0, ir_js_1.tuple)([]), binds: [] }),
}), def({
    name: 'elementOf',
    arity: 1,
    ...
    lower: ([t], ctx) => {
        ...
        return expr((0, ir_js_1.cond)(t, (0, ir_js_1.ref)('ReadonlyArray', [(0, ir_js_1.infer)(i)]), (0, ir_js_1.ref)(i), (0, ir_js_1.kw)('never')));
    },
}), def({
    name: 'arrayOf',
    arity: 1,
    ...
    lower: ([t]) => expr((0, ir_js_1.indexed)(t, (0, ir_js_1.kw)('number'))),
}), def({
    name: 'reverse',
    arity: 1,
    ...
    lower: ([t], ctx) => {
        ...
        return expr((0, ir_js_1.ref)('Reverse', [t]));
    },
}), def({
    name: 'join',
    arity: 2,
    ...
    lower: ([t, sep], ctx) => {
        ctx.usePrelude('Join');
        return expr((0, ir_js_1.ref)('Join', [t, sep]));
    },
}));
// ---------------------------------------------------------------------------
// Object
...
register(def({
    ...
    arity: 2,
    doc: 'get(o, k) -> O[K]',
    lower: ([o, k]) => expr((0, ir_js_1.indexed)(o, k)),
}), def({
    name: 'pick',
    arity: 2,
    doc: 'pick(o, k) -> Pick<O, K>',
    lower: ([o, k]) => expr((0, ir_js_1.ref)('Pick', [o, k])),
}), def({
    name: 'omit',
    arity: 2,
    doc: 'omit(o, k) -> Omit<O, K>',
    lower: ([o, k]) => expr((0, ir_js_1.ref)('Omit', [o, k])),
}), def({
    name: 'merge',
    arity: [2, 16],
    doc: 'merge(a, b, ...) -> A & B & ...',
    lower: (args) => expr((0, ir_js_1.intersection)(args.map((a) => a))),
}), def({
    name: 'anyOf',
    arity: [2, 32],
    doc: 'anyOf(a, b, ...) -> A | B | ...',
    lower: (args) => expr((0, ir_js_1.union)(args.map((a) => a))),
}), def({
    name: 't',
    arity: 0,
    ...
    lower: ([o], ctx) => {
        ...
        return expr((0, ir_js_1.ref)('Simplify', [o]));
    },
}));
// ---------------------------------------------------------------------------
// Predicates & meta
...
register(def({
    ...
    lower: ([x]) => ({
        tag: 'match',
        check: (0, ir_js_1.tuple)([{ expr: x }]),
        ext: (0, ir_js_1.tuple)([{ expr: (0, ir_js_1.kw)('never') }]),
        binds: [],
    }),
}), def({
    name: 'isAny',
    ...
    lower: ([x]) => ({
        ...
        ext: (0, ir_js_1.intersection)([{ kind: 'lit', value: 1 }, x]),
        binds: [],
    }),
}), def({
    name: 'equals',
    ...
    lower: ([a, b], ctx) => {
        ...
        return expr((0, ir_js_1.ref)('Equals', [a, b]));
    },
}), def({
    name: 'error',
    arity: 1,
    ...
    lower: ([m], ctx) => {
        ...
        return expr((0, ir_js_1.ref)('ScriptTypeError', [m]));
    },
}), def({
    name: 'voidType',
    arity: 0,
    doc: 'voidType() -> void  (`void` is a JS operator, so it needs a call form)',
    lower: () => expr((0, ir_js_1.kw)('void')),
}), def({
    name: 'fnType',
    arity: 2,
    ...
    lower: ([x]) => expr((0, ir_js_1.cond)((0, ir_js_1.tuple)([{ expr: x }]), (0, ir_js_1.tuple)([{ expr: (0, ir_js_1.kw)('unknown') }]), x, (0, ir_js_1.kw)('never'))),
}));
 ...
 */
function guardEquivalent(guard, inferring) {
    ...
    const strip = (e, inTemplate) => {
        if (e.kind === 'infer')
            return e.constraint ?? (inTemplate ? (0, ir_js_1.kw)('string') : (0, ir_js_1.kw)('unknown'));
        return e;
    };

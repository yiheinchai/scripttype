...
function formatAlias(a, opts = {}) {
    ...
    const flat = `${head}${paramsFlat} = ${(0, ir_js_1.emit)(a.body)}`;
...
function emitParam(p) {
    ...
    const con = p.constraint ? ` extends ${(0, ir_js_1.emit)(p.constraint)}` : '';
    const def = p.default ? ` = ${(0, ir_js_1.emit)(p.default)}` : '';
...
function fmt(e, start, anchor, minPrec, width, step) {
    ...
    switch (e.kind) {
        case 'conditional': {
            ...
            const tail = cur;
            const guardOf = (a) => `${(0, ir_js_1.emit)(a.check, ir_js_1.P_COND + 1)} extends ${(0, ir_js_1.emit)(a.ext, ir_js_1.P_COND + 1)}`;
            ...
            arms.forEach((a, i) => {
                ...
                const thenFlat = (0, ir_js_1.emit)(a.then);
            ...
            const tailFlat = (0, ir_js_1.emit)(tail);
        ...
        case 'intersection': {
            ...
            const prec = (e.kind === 'union' ? ir_js_1.P_UNION : ir_js_1.P_INTER) + 1;
            return e.members
        ...
        case 'object': {
            ...
            if (e.index) {
                const label = `[key: ${(0, ir_js_1.emit)(e.index.key)}]: `;
        ...
        case 'mapped': {
            ...
            const as = e.as ? ` as ${(0, ir_js_1.emit)(e.as)}` : '';
            const label = `${ro}[${e.param} in ${(0, ir_js_1.emit)(e.constraint)}${as}]${opt}: `;
        ...
        case 'tuple': {
            const parts = e.elements.map((el) => {
                ...
                const inner = fmt(el.expr, body + lead.length, body, el.spread ? ir_js_1.P_PREFIX : 0, width, step);
        ...
        case 'indexed':
            return `${fmt(e.obj, start, anchor, ir_js_1.P_POSTFIX, width, step)}[${(0, ir_js_1.emit)(e.index)}]`;
        case 'op':
            return `${e.op} ${fmt(e.target, start + e.op.length + 1, anchor, ir_js_1.P_PREFIX + 1, width, step)}`;
        case 'array':
            return `${e.readonly ? 'readonly ' : ''}${fmt(e.element, start, anchor, ir_js_1.P_POSTFIX, width, step)}[]`;
        case 'paren': {
        ...
        case 'fn': {
            ...
            const args = e.params.map((p, i) => (0, ir_js_1.emitFnParam)(e, p, i));

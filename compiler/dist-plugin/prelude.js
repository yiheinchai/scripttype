...
exports.providedBy = exports.PRELUDE = void 0;
exports.resolvePrelude = resolvePrelude;
exports.PRELUDE = {
    ScriptTypeError: {
...
function resolvePrelude(used) {
    ...
    const visit = (name) => {
        if (seen.has(name))
            return;
        seen.add(name);
        const entry = exports.PRELUDE[name];
        if (!entry)
            return;
        for (const d of entry.deps ?? [])
            visit(d);
        out.push({ source: entry.source, provides: (0, exports.providedBy)(name) });
    };
    for (const n of used)
        visit(n);
    return out;

...
for (const n of txtOnly) {
    ...
    try {
        ...
        if (/heap out of memory|Ineffective mark-compacts/i.test(txt))
            why = 'checker OOM';
        else if (/Maximum call stack/i.test(txt))
            why = 'checker stack overflow';
        else if (/command not found/i.test(txt))
            why = 'runner error';
        else {
            const first = txt.split('\n').find((l) => /error|Error/.test(l));
            if (first)
                why = first.slice(0, 60);
        }
    }
    catch {
        /* keep default */
...
const sum = (k) => rows.reduce((a, r) => a + (r.counts.get(k) ?? 0), 0);
...
for (const r of [...rows].sort((a, b) => b.total - a.total)) {
    ...
    const j = r.total - u;
    lines.push(`| ${r.repo} | ${r.total} | ${r.covered} | ${pct(r.covered, r.total)} | ${pct(r.covered, j)} | ` +
        `${r.counts.get('raw') ?? 0} | ${r.counts.get('mismatch') ?? 0} | ${u} |`);
}
lines.push(`| **TOTAL** | **${total}** | **${covered}** | **${pct(covered, total)}** | **${pct(covered, judgeable)}** | ` +

...
function cachedLibFile(fileName, lv) {
...
const isMissingName = (d) => /^Cannot find name|^Cannot find module|is not generic/.test(d);
...
export function batchFile(rel) {
    ...
    if (!entries.length)
        return [];
    const outcomes = [];
    const files = new Map([['/eq.ts', EQ]]);
    const checks = [`import type { __Eq } from './eq.js'`];
    const live = [];
    entries.forEach((e, i) => {
        ...
        const inst = (ns) => (args.length ? `${ns}${i}.${e.name}<${args.join(', ')}>` : `${ns}${i}.${e.name}`);
    ...
    for (const { name, idx } of live) {
        ...
        if (cDiags.length) {
            const status = cDiags.every(isMissingName) ? 'unresolved-deps' : 'compile-error';
...
export function collectFiles(relRoot) {
    ...
    for (const root of roots) {
        for (const f of collectFiles(root)) {
            try {
                ...
                console.error(`  error in ${f}: ${err.message.split('\n')[0]}`);
            }
        }
    }

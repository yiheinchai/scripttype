...
for (const dir of dirs) {
    ...
    const tc = typecheckScriptType({ [`${name.replace(/\//g, '__')}.st.ts`]: fs.readFileSync(stPath, 'utf8') }, deps);
    if (tc.ok)
        gateOk++;
    else
        gateBad.push(`${name}: ${tc.errors[0] ?? tc.suppressions[0]}`);
...
for (const b of gateBad)
    console.log(`  FAIL ${b.slice(0, 160)}`);
console.log(`type identity:   ${verifyOk}/${total}`);
for (const b of verifyBad)
    console.log(`  FAIL ${b.slice(0, 160)}`);

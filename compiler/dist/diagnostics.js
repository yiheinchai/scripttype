...
export function diagnosticFromNode(code, message, node, help) {
...
export function formatDiagnostic(d, opts = {}) {
    ...
    const help = d.help ?? CATALOGUE[d.code]?.help;
    if (help)
...
export function explain(code, color = colorsEnabled()) {
    const spec = CATALOGUE[code.toUpperCase()];
    if (!spec)
        return undefined;
    const c = paint(color);
    return [

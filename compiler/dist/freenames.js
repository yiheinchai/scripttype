...
const AMBIENT_NAMES = (() => {
    ...
    try {
        ...
        for (const m of text.matchAll(/declare (?:function|const) ([A-Za-z_$][A-Za-z0-9_$]*)/g))
            out.add(m[1]);
        for (const m of text.matchAll(/^type ([A-Za-z_$][A-Za-z0-9_$]*)/gm))
            out.add(m[1]);
    }
    catch {
        /* no ambient file yet */
...
export function freeNames(source) {
    ...
    const noteUsed = (n) => {
        ...
        if (ts.isIdentifier(n)) {
            ...
            const isPropAssignKey = parent && (ts.isPropertyAssignment(parent) || ts.isPropertySignature(parent)) && parent.name === n;
            if (!isPropertyName && !isPropAssignKey)
                used.add(n.text);
        }
    ...
    const noteTypeUse = (n) => {
        if (ts.isTypeReferenceNode(n))
            typesUsed.add(n.typeName.getText(sf).split('.')[0]);
...
const GENERIC = '<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any,' +
...
function qualifiedMembers(sf) {
    ...
    const add = (text) => {
        ...
        const root = parts[0];
        if (!out.has(root))
            out.set(root, new Set());
        out.get(root).add(parts[1]);
    };
    const walk = (n) => {
        if (ts.isTypeReferenceNode(n))
            add(n.typeName.getText(sf));
        if (ts.isTypeQueryNode(n))
            add(n.exprName.getText(sf));
 ...
 */
export function ambientFor(source) {
    const sf = ts.createSourceFile('gen.st.ts', source, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
    ...
    for (const root of nsRoots) {
        ...
        for (const m of [...quals.get(root)].sort())
            lines.push(`  export type ${m}${GENERIC} = any`);
        lines.push(`}`);
    }
    for (const n of free.values)
        lines.push(`declare const ${n}: any`);
    for (const n of free.types)
        lines.push(`type ${n}${GENERIC} = any`);
    void nsSet;
    if (!lines.length)
        return '';
    return [
...
/** Ambient declarations making a generated file self-contained. */
export function declarePreamble(source) {
 ...
 * parameter as free.
 */
export function declarePreambleForTypes(source) {
    ...
    const noteDeclared = (n) => {
        ...
        if (ts.isInferTypeNode(n))
            declared.add(n.typeParameter.name.text);
        if (ts.isMappedTypeNode(n))
            declared.add(n.typeParameter.name.text);
        if (ts.isModuleDeclaration(n) && ts.isIdentifier(n.name))
            declared.add(n.name.text);
        ts.forEachChild(n, noteDeclared);
    };
    noteDeclared(sf);
    const noteUsed = (n) => {
        if (ts.isTypeReferenceNode(n))
            used.add(n.typeName.getText(sf).split('.')[0]);
        if (ts.isTypeQueryNode(n))
            used.add(n.exprName.getText(sf).split('.')[0]);
        ts.forEachChild(n, noteUsed);
    };
    noteUsed(sf);
    const free = [...used]
...
export function freeNamesOf(source) {

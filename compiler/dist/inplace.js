...
function cachedFile(fileName, lv) {
...
function suffixAliases(code, names, suffix) {
    const map = new Map();
...
function declaredNames(code) {
    const out = [];
    for (const m of code.matchAll(/^(?:export )?type ([A-Za-z_$][A-Za-z0-9_$]*)/gm))
        out.push(m[1]);
    return out;
}
...
function buildOverlay(rel) {
    ...
    if (!entries.length)
        return undefined;
    const outcomes = [];
    const appended = ['', '// ===== ScriptType round-trip (generated) =====', EQ_DECL];
    const live = [];
    entries.forEach((e, i) => {
        ...
        const qualify = (n) => (e.ns.length ? `${e.ns.join('.')}.${n}` : n);
        const args = witnessArgs(e.decl, sf);
        const inst = (n) => (args.length ? `${qualify(n)}<${args.join(', ')}>` : qualify(n));
        appended.push(`type __st_EQ${i} = ${EQ_NAME}<${inst(e.name)}, ${inst(`${e.name}${suffix}`)}>`);
        live.push({ name: e.name, idx: i, ns: e.ns });
    });
    if (!live.length)
        return { rel, abs, text: realText, offset: realText.length, live: [], decided: outcomes };
    return {
        rel,
        abs,
...
function checkOverlays(overlays) {
    const out = [];
    ...
    const host = {
        ...
        getSourceFile: (fileName, lv) => {
            ...
            if (ov)
                return ts.createSourceFile(fileName, ov.text, lv, true);
    ...
    for (const o of active)
        out.push(...readOutcomes(program, checker, o));
    return out;
}
function readOutcomes(program, checker, o) {
    const outcomes = [];
    ...
    const indexAliases = (n) => {
        if (ts.isTypeAliasDeclaration(n) && n.getStart() >= offset)
            aliases.set(n.name.text, n);
 ...
 */
export function inplaceFiles(rels, batchSize = 20, onBatch) {
    const out = [];
    const overlays = [];
...
export function inplaceFile(rel) {
...
if (process.argv[1] && import.meta.filename === path.resolve(process.argv[1])) {
    ...
    if (jsonIdx >= 0)
        skipValues.add(args[jsonIdx + 1]);
    if (shardIdx >= 0)
        skipValues.add(args[shardIdx + 1]);
    const roots = args.filter((a, i) => !a.startsWith('--') &&
    ...
    const writeSoFar = (soFar) => {
        if (jsonOut)
            fs.writeFileSync(jsonOut, JSON.stringify(soFar));
    };
    const all = inplaceFiles(mine, 20, writeSoFar);

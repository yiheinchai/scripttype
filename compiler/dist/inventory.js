...
export function analyzeRepo(repoDir, repoLabel) {
    const stats = {
        repo: repoLabel,
        files: 0,
        generic: 0,
            ...
            if (ts.isTypeAliasDeclaration(n)) {
                ...
                const scan = (x) => {
                    ...
                    if (ts.isTemplateLiteralTypeNode(x))
                        hasTpl = true;
                    if (ts.isTypeReferenceNode(x) && x.typeName.getText(sf) === name)
                        selfRef = true;
...
export function inventory() {
    ...
    for (const cat of fs.readdirSync(REPO_ROOT, { withFileTypes: true })) {
        if (!cat.isDirectory() || !/^\d\d-/.test(cat.name))
            continue;
...
if (process.argv[1] && import.meta.filename === path.resolve(process.argv[1])) {
    ...
    for (const s of stats.sort((a, b) => b.generic - a.generic)) {
        ...
        tr += s.recursive;
        tcov += cov;
        console.log(s.repo.padEnd(42) + pad(s.generic, 8) + pad(s.conditional, 6) + pad(s.mapped, 7) + pad(s.templateLit, 5) + pad(s.recursive, 5) + pad(cov || '', 9));
    }
    console.log('-'.repeat(82));

...
// Constructors
// ---------------------------------------------------------------------------
export const ref = (name, args) => args && args.length ? { kind: 'ref', name, args } : { kind: 'ref', name };
export const kw = (name) => ({ kind: 'keyword', name });
export const str = (value) => ({ kind: 'lit', value, str: true });
export const num = (value) => ({ kind: 'lit', value });
export const bool = (value) => ({ kind: 'lit', value });
export const NEVER = kw('never');
export const UNKNOWN = kw('unknown');
export const tuple = (elements) => ({ kind: 'tuple', elements });
export const infer = (name, constraint) => constraint ? { kind: 'infer', name, constraint } : { kind: 'infer', name };
export const indexed = (obj, index) => ({ kind: 'indexed', obj, index });
export const raw = (text) => ({ kind: 'raw', text });
export const cond = (check, ext, then, els) => ({
    kind: 'conditional',
    check,
    ext,
    then,
...
export function intersection(members) {
    const flat = [];
    for (const m of members) {
 ...
 */
export function template(quasis, exprs) {
    const q = [quasis[0] ?? ''];
    const e = [];
...
export function precedenceOf(e) {
    switch (e.kind) {
        case 'conditional':
        case 'fn':
            return P_COND;
 ...
 * a rest parameter.
 */
export function emitFnParam(fn, p, i) {
    const rest = fn.hasRest && i === fn.params.length - 1 ? '...' : '';
    return `${rest}a${i}: ${emit(p)}`;
}
export function emit(e, minPrec = 0) {

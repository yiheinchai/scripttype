...
import { NEVER, cond, countRefs, emit, indexed, infer as inferNode, intersection, kw, num, bool, ref, str, substitute, template, tuple, union, } from './ir.js';
import { BUILTINS, guardEquivalent } from './builtins.js';
...
function unsupportedExpression(e) {
    const hints = {
        [ts.SyntaxKind.ArrowFunction]: 'Type-level code has no closures; declare a top-level function and call it by name.',
        [ts.SyntaxKind.FunctionExpression]: 'Type-level code has no closures; declare a top-level function and call it by name.',
        [ts.SyntaxKind.AwaitExpression]: 'There is nothing to await at the type level.',
        [ts.SyntaxKind.NewExpression]: 'Use `ctorType(params, ret)` for a constructor type, or name the type with `t<T>()`.',
        [ts.SyntaxKind.RegularExpressionLiteral]: 'Match with a template-literal pattern instead: `matches<`${Hole<"A">}-${Hole<"B">}`>(x)`.',
        [ts.SyntaxKind.TypeOfExpression]: 'In an expression a name already denotes its type; `typeof` is only needed inside a type annotation.',
    };
    return [
...
export function compileSourceFile(sf, opts = {}) {
    const aliases = [];
...
const hasExport = (n) => !!(ts.getCombinedModifierFlags(n) & ts.ModifierFlags.Export);
...
class FunctionCompiler {
    ...
    applyMutation(e, vars) {
        ...
        if (ts.isCallExpression(e) && ts.isPropertyAccessExpression(e.expression)) {
            ...
            if (ts.isIdentifier(target)) {
                ...
                if (method === 'concat') {
                    vars.set(target.text, tuple([{ expr: cur, spread: true }, ...args.map((a) => ({ expr: a, spread: true }))]));
                    return;
                }
            }
        }
    ...
    bindArrayPattern(pattern, initArg, vars, k) {
        ...
        for (const e of elements)
            if (e.name)
                v2.set(e.name, ref(e.fresh));
        const ext = tuple(elements.map((e) => ({
    ...
    lowerTest(e, vars, onTrue, onFalse) {
        ...
        if (ts.isBinaryExpression(e)) {
            ...
            if (op === ts.SyntaxKind.InKeyword) {
                return cond(this.expr(e.left, vars), { kind: 'op', op: 'keyof', target: this.expr(e.right, vars) }, onTrue(vars), onFalse(vars));
            }
            ...
            if (op === ts.SyntaxKind.LessThanToken ||
                ...
                throw new CompileError(`\`${ts.tokenToString(op)}\` has no type-level meaning`, e, 'ST1500', 'TypeScript cannot compare numbers in the type system. Compare tuple lengths ' +
    ...
    tryTypeofNarrowing(e, vars) {
        ...
        const equality = op === ts.SyntaxKind.EqualsEqualsEqualsToken || op === ts.SyntaxKind.EqualsEqualsToken;
        if (!equality && !negated)
            return undefined;
        ...
        const [typeofSide, literalSide] = ts.isTypeOfExpression(e.left)
            ...
            : ts.isTypeOfExpression(e.right)
                ? [e.right, e.left]
                : [undefined, undefined];
        if (!typeofSide || !literalSide || !ts.isStringLiteral(literalSide))
            return undefined;
    ...
    lowerLoop(stmt, vars, after, _outerLoop) {
        ...
        const guardNames = ts.isWhileStatement(stmt) ? collectRead(stmt.expression) : new Set();
        ...
        const callWith = (v) => {
            const args = [];
            if (listVar)
                args.push(v.get(`__list__${helperName}`) ?? ref(listVar));
            for (const a of accumNames)
                args.push(v.get(a) ?? ref(a));
            for (const c of carried)
                args.push(v.get(c) ?? ref(c));
        ...
        if (ts.isForOfStatement(stmt)) {
            ...
            helperBody = cond(ref(listVar), tuple([
    ...
    lowerForIn(stmt, vars, k) {
        ...
        if (!ts.isBinaryExpression(assign) ||
        ...
        const isKeySet = ts.isCallExpression(iterExpr) &&
    ...
    tryFuseGuard(guardExpr, bodyStmts, helperVars, recurse, loopCtx, after) {
        ...
        if (!d || !d.initializer || !ts.isArrayBindingPattern(d.name))
            return undefined;
    ...
    expr(e, vars) {
        ...
        if (ts.isBinaryExpression(e)) {
            const op = e.operatorToken.kind;
            if (op === ts.SyntaxKind.BarToken)
                return union([this.expr(e.left, vars), this.expr(e.right, vars)]);
            ...
            if (op === ts.SyntaxKind.EqualsEqualsEqualsToken ||
                ...
                return this.lowerTest(e, vars, () => bool(true), () => bool(false));
            }
        }
        ...
        if (ts.isObjectLiteralExpression(e)) {
            const props = e.properties.map((p) => {
                if (ts.isPropertyAssignment(p)) {
                    ...
                    const name = computed
                        ? emit(this.expr(p.name.expression, vars))
                        : propName(p.name);
        ...
        if (ts.isPropertyAccessExpression(e)) {
            ...
            if (e.name.text === 'length')
                return indexed(this.expr(e.expression, vars), str('length'));
    ...
    typeNode(t, vars) {
        ...
        if (ts.isTupleTypeNode(t)) {
            return tuple(t.elements.map((el) => ts.isRestTypeNode(el)
                ? { expr: this.typeNode(el.type, vars), spread: true }
                : { expr: this.typeNode(el, vars) }));
        }
        if (ts.isUnionTypeNode(t))
            return union(t.types.map((x) => this.typeNode(x, vars)));
        if (ts.isIntersectionTypeNode(t))
            return intersection(t.types.map((x) => this.typeNode(x, vars)));
        if (ts.isLiteralTypeNode(t)) {
            const l = t.literal;
            if (ts.isStringLiteral(l))
                return str(l.text);
            if (ts.isNumericLiteral(l))
                return num(Number(l.text));
        ...
        if (ts.isTypeQueryNode(t)) {
            ...
            if (ts.isQualifiedName(t.exprName)) {
                ...
                if (this.markers.has(root))
                    return ref(t.exprName.right.text);
            }
        ...
        if (ts.isRestTypeNode(t))
            return this.typeNode(t.type, vars);
        if (ts.isOptionalTypeNode(t))
            return this.typeNode(t.type, vars);
        if (ts.isNamedTupleMember(t))
            return this.typeNode(t.type, vars);

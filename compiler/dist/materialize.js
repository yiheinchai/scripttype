...
if (fs.existsSync(statusDir)) {
    for (const f of fs.readdirSync(statusDir)) {
        ...
        for (const o of list) {
            if (!statuses.has(o.file))
                statuses.set(o.file, new Map());
...
for (const rel of targetFiles.sort()) {
    ...
    if (!entries.length)
        continue;
    files++;
    const byName = statuses.get(rel) ?? new Map();
    const blocks = [];
    const counts = new Map();
    for (const e of entries) {
        const o = byName.get(e.name);
        const status = o?.status ?? 'unresolved-deps';
...
for (const [s, n] of [...perStatus].sort((a, b) => b[1] - a[1]))

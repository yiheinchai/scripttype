...
export function verify(c) {
    ...
    const assertions = c.samples
        ...
        const inst = (ns) => (args.trim() ? `${ns}.${c.typeName}<${args}>` : `${ns}.${c.typeName}`);
        return (`export type __O${i} = ${inst('O')}\n` +
            `export type __C${i} = ${inst('C')}\n` +
            `export type __EQ${i} = __Equals<${inst('O')}, ${inst('C')}>`);
    })
        .join('\n');
    files.set('/check.ts', `import type * as O from './original.js'\n` +
    ...
    try {
        ...
        return {
            ...
            originalDiagnostics: [],
            samples: [],
            error: `failed to create program: ${e.message}`,
        };
    }
    ...
    const samples = c.samples.map((args, i) => {
    ...
    const ok = compiledDiagnostics.length === 0 &&
...
export function formatResult(r) {
    if (r.ok)
        return `PASS  ${r.name}  (${r.samples.length} samples)`;
    const lines = [`FAIL  ${r.name}`];
    if (r.error)
        lines.push(`  error: ${r.error}`);
    for (const d of r.originalDiagnostics)
        lines.push(`  reference type error: ${d}`);
    for (const d of r.compiledDiagnostics)
        lines.push(`  compiled type error: ${d}`);

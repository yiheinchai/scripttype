 ...
 */
function typeParamNames(tps: TypeExpr, who: string): string[] {
  if (tps.kind !== 'tuple') throw new Error(`${who}(typeParams, params, ret) needs a tuple of type parameters`)
...
function signatureBuiltins() {
  const forms: { name: string; sig: SignatureKind; isCtor: boolean; shape: string }[] = [
  ...
  const named = (sig: SignatureKind) => (sig === 'method' ? 1 : 0)
 ...
 */
function leadingName(args: TypeExpr[], base: number, who: string): { sigName?: string; rest: TypeExpr[] } {
  if (args.length <= base) return { rest: args }
  const first = args[0]!
  if (first.kind !== 'lit' || !first.str) throw new Error(`${who}'s member name must be a string literal`)
...
function paramIndex(e: TypeExpr, who: string): number {
...
register(
  ...
  def({
    ...
    lower: ([tps, params, ret]) =>
      expr({ ...fnNode(params!, ret!, false), typeParams: typeParamNames(tps!, 'genericFnType') }),
  }),
  ...signatureBuiltins(),
  def({
    name: 'paramIs',
  ...
  def({
    ...
    lower: ([i, t]) => {
      const node: TypeExpr = { kind: 'predicate', param: paramIndex(i!, 'paramAsserts'), asserts: true }

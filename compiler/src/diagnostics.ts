/**
 * ScriptType diagnostics.
 *
 * A type-level language is only adoptable if its failure mode is a *teaching* moment.
 * TypeScript's own type errors are the single most cited reason advanced type code is
 * painful, so ScriptType cannot afford to answer a mistake with `unknown variable 'x'`
 * and a stack trace into the compiler's own source.
 *
 * Every compiler error therefore carries:
 *   - a stable code (`ST1012`) that can be looked up with `scripttype explain ST1012`,
 *   - a span in the user's file, rendered as a source frame with a caret,
 *   - a one-line `help:` telling the user what to write instead.
 *
 * The long-form `explain` text is the language documentation for that construct. It is
 * kept here rather than in prose docs so it cannot drift from the code that raises it.
 */
import ts from 'typescript'

export type DiagCode = keyof typeof CATALOGUE

export interface DiagnosticSpec {
  /** Short imperative fix, shown inline under the source frame. */
  help: string
  /** Long-form explanation with a worked example, shown by `scripttype explain`. */
  explain: string
}
...
/** A resolved diagnostic, ready to render. */
export interface Diagnostic {
  code: DiagCode | string
  message: string
  file?: string
  /** Absolute offset into `text`. */
  start?: number
  length?: number
  /** Overrides the catalogue help, when the site has something more specific to say. */
  help?: string
...
export function diagnosticFromNode(
  ...
  const d: Diagnostic = { code, message, help, severity: 'error' }
  if (node) {
...
function paint(on: boolean) {
  const w = (c: string) => (s: string) => (on ? `${c}${s}${ANSI.reset}` : s)
  return {
    bold: w(ANSI.bold),
    dim: w(ANSI.dim),
    red: w(ANSI.red),
...
export interface FormatOptions {
  /** The source text the span refers to, used to draw the frame. */
  text?: string
  color?: boolean
  /** Lines of context above and below the span. */
  context?: number
}
...
export function formatDiagnostic(d: Diagnostic, opts: FormatOptions = {}): string {
  ...
  if (text !== undefined && d.start !== undefined) {
    ...
    for (let i = first; i <= last; i++) {
      ...
      if (i === line) {
        ...
        out.push(
          `${c.dim(pad('') + ' |')} ${' '.repeat(col)}${c.red('^'.repeat(width))}`,
        )
      }
    }
    out.push(c.dim(`${pad('')} |`))
  ...
  const help = d.help ?? CATALOGUE[d.code as DiagCode]?.help
  if (help) out.push(`${c.green(c.bold('help'))}: ${help}`)
  if (CATALOGUE[d.code as DiagCode]) {
...
export function explain(code: string, color = colorsEnabled()): string | undefined {
  const spec = CATALOGUE[code.toUpperCase() as DiagCode]

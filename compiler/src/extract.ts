...
export interface ExtractResult {
  /** Self-contained source text: dependencies first, then the requested type. */
  source: string
  /** Names emitted, in dependency order. */
  emitted: string[]
  /** Each emitted declaration separately, so callers can take deps without re-extracting. */
  parts: { name: string; text: string }[]
  /** Referenced names that were not found locally (lib types, cross-file imports). */
  unresolved: string[]
}
...
export function extractType(filePath: string, typeName: string): ExtractResult {
  ...
  if (!decls.has(typeName)) {
    throw new Error(
      `type '${typeName}' not found in ${filePath}\n` +
  ...
  const collect = (name: string) => {
    ...
    if (!decl) {
      if (!LIB_TYPES.has(name)) unresolved.add(name)
      return
    }
    ...
    const collectBound = (n: ts.Node) => {
      if (ts.isTypeParameterDeclaration(n)) params.add(n.name.text)
      if (ts.isInferTypeNode(n)) params.add(n.typeParameter.name.text)
      if (ts.isMappedTypeNode(n)) params.add(n.typeParameter.name.text)
    ...
    const walk = (n: ts.Node) => {
      if (ts.isTypeReferenceNode(n)) {
        ...
        if (root && root !== name && !params.has(root)) refs.push(root)
      }
      ts.forEachChild(n, walk)
    }
    walk(decl)
    for (const r of refs) collect(r)
    emitted.push(name)
  }
  collect(typeName)
...
export function corpusPath(root: string, rel: string): string {
  const p = `${root}/${rel}`
  if (!fs.existsSync(p)) throw new Error(`corpus file not found: ${p}`)
  return p
}

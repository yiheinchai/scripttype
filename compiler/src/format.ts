...
const BRACKETED = new Set<TypeExpr['kind']>(['object', 'mapped', 'tuple', 'ref'])

/** Render a declaration, wrapping where it does not fit. */
export function formatAlias(a: TypeAlias, opts: FormatOptions = {}): string {
  ...
  const params =
    paramsFlat && head.length + paramsFlat.length + 3 > width
  ...
  if (!params.includes('\n')) {
    ...
    if (BRACKETED.has(a.body.kind)) return doc + prefix + fmt(a.body, prefix.length, 0, 0, width, step)
  }
...
function emitParam(p: TypeAlias['params'][number]): string {
...
function fmt(
  ...
  if (start + flat.length <= width) return flat
  ...
  switch (e.kind) {
    case 'conditional': {
      ...
      const arms: { check: TypeExpr; ext: TypeExpr; then: TypeExpr }[] = []
      let cur: TypeExpr = e
      ...
      const tail = cur
      const guardOf = (a: (typeof arms)[number]) =>
        `${emit(a.check, P_COND + 1)} extends ${emit(a.ext, P_COND + 1)}`

      // `?` and `:` sit one step in from the anchor; their operands hang off them.
      const q = body
      const qPad = sp(q)
      const branch = (x: TypeExpr) => fmt(x, q + 2, q, 0, width, step)
      ...
      lines.push(
        anchor + 2 + tailFlat.length <= width
          ? `: ${tailFlat}`
    ...
    case 'mapped': {
      const ro =
        e.readonly === true ? 'readonly ' : e.readonly === '+' ? '+readonly ' : e.readonly === '-' ? '-readonly ' : ''

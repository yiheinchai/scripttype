 ...
 */
import { type TypeExpr, countRefs, emit, kw } from './ir.js'
import { guardEquivalent } from './builtins.js'
 ...
 */
export function fuseGuards(e: TypeExpr): TypeExpr {
  const rec = fuseGuards
  if (e.kind !== 'conditional') return mapChildren(e, rec)

  const outer = { ...e, check: rec(e.check), then: rec(e.then), else: rec(e.else) }
  const inner = outer.then
  if (
    inner.kind === 'conditional' &&
    emit(inner.check) === emit(outer.check) &&
    inner.else.kind === 'keyword' &&
    inner.else.name === 'never' &&
    guardEquivalent(outer.ext, inner.ext)
  ) {
    return fuseGuards({
      kind: 'conditional',
      check: outer.check,
 ...
 */
export function pruneUnusedInfers(e: TypeExpr): TypeExpr {
...
function replaceUnused(ext: TypeExpr, then: TypeExpr, inTemplate: boolean): TypeExpr {
  switch (ext.kind) {
    case 'infer': {
...
function mapChildren(e: TypeExpr, f: (x: TypeExpr) => TypeExpr): TypeExpr {
  switch (e.kind) {
    case 'ref':
...
export function optimize(e: TypeExpr): TypeExpr {

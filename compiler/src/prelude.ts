...
export interface PreludeEntry {
  source: string
  deps?: string[]
  /** Every type name this entry declares. Defaults to the registry key. */
  provides?: string[]
}

export const PRELUDE: Record<string, PreludeEntry> = {
  ScriptTypeError: {
  ...
  SplitLast: {
    source:
      `type SplitLast<S extends string, Sep extends string> =\n` +
  ...
  StrLength: {
    ...
    source:
      `type StrLengthAcc<S extends string, Acc extends unknown[] = []> =\n` +
...
export const providedBy = (name: string): string[] => PRELUDE[name]?.provides ?? [name]

/** Resolve the transitive closure of used prelude helpers, in dependency order. */
export function resolvePrelude(used: Iterable<string>): { source: string; provides: string[] }[] {
  const out: { source: string; provides: string[] }[] = []
  ...
  const visit = (name: string) => {
    ...
    if (!entry) return
    for (const d of entry.deps ?? []) visit(d)
    out.push({ source: entry.source, provides: providedBy(name) })
  }
  for (const n of used) visit(n)
  return out
}

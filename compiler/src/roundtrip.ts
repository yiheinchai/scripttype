...
export interface RoundTripOutcome {
  name: string
  status: 'covered' | 'raw' | 'compile-error' | 'mismatch' | 'reference-error'
  gaps: string[]
  detail?: string
  scriptType?: string
  emitted?: string
...
export function compareAliases(
  ...
  if (compiledDiags.length) return { equal: false, detail: `compiled: ${compiledDiags[0]}` }

  const originalSf = program.getSourceFile('/original.ts')!
  const originalDiags = program.getSemanticDiagnostics(originalSf).map(fmt)
  if (originalDiags.length) return { equal: false, detail: `reference: ${originalDiags[0]}` }
...
function witnessArgs(decl: ts.TypeAliasDeclaration, sf: ts.SourceFile): string[] {
  return (decl.typeParameters ?? []).map((tp) => {
    ...
    if (/\bkeyof\b|\binfer\b/.test(c)) return 'any'
    return c
  })
}

export function roundTripFile(rel: string, opts: { only?: string; limit?: number } = {}): RoundTripOutcome[] {
  ...
  if (opts.only) entries = entries.filter((e) => e.name === opts.only)
  ...
  for (const e of entries) {
    ...
    try {
      ...
      if (depParts.length) deps = depParts.map((p) => p.text).join('\n\n') + '\n\n'
    } catch (err) {
      out.push({ name: e.name, status: 'reference-error', gaps, detail: (err as Error).message })
      continue
    }
...
if (process.argv[1] && import.meta.filename === path.resolve(process.argv[1])) {
  ...
  for (const r of results) {
    ...
    let line = `${tag} ${r.name}`
    if (r.status === 'raw') line += `  [${[...new Set(r.gaps)].join('; ')}]`
    else if (r.detail) line += `  ${r.detail.slice(0, 160)}`
    console.log(line)
  ...
  console.log(`\n${rel}`)
  console.log(
    `  ${covered}/${total} round-trip equivalent` +
      [...byStatus].filter(([k]) => k !== 'covered').map(([k, v]) => `, ${v} ${k}`).join(''),
  )
  const allGaps = new Map<string, number>()
  for (const r of results) for (const g of r.gaps) allGaps.set(g, (allGaps.get(g) ?? 0) + 1)
  if (allGaps.size) {
    console.log('  language gaps:')
    for (const [g, n] of [...allGaps].sort((a, b) => b[1] - a[1])) console.log(`    ${n.toString().padStart(4)}  ${g}`)
  }
}

...
if (!fs.existsSync(metaPath)) {
  fs.writeFileSync(
    metaPath,
    JSON.stringify({ name, sourcePath: rel, typeName, samples }, null, 2) + '\n',
  )
}
const stPath = path.join(dir, 'source.st.ts')
if (!fs.existsSync(stPath)) fs.writeFileSync(stPath, '')

console.log(`scaffolded ${name}`)
console.log(`  emitted deps: ${emitted.join(', ')}`)
if (unresolved.length) console.log(`  UNRESOLVED (fixture will not typecheck): ${unresolved.join(', ')}`)

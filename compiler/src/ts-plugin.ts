...
function init(mod: { typescript: typeof tsModule }) {
  ...
  function create(info: tsModule.server.PluginCreateInfo) {
    ...
    proxy.getCodeFixesAtPosition = (fileName, start, end, errorCodes, formatOptions, prefs) => {
      ...
      if (!IS_SCRIPTTYPE.test(fileName)) return prior
      if (!errorCodes.some((c) => c >= CODE_BASE)) return prior
      ...
      try {
        ...
        log(`code fixes failed for ${fileName}: ${(e as Error).message}`)
        return prior
      }
    }
...
function enclosingStatement(ts: typeof tsModule, node: tsModule.Node): tsModule.Node {
  let n: tsModule.Node = node
...
function codeFixes(
...
): tsModule.CodeFixAction[] {
  const { errors } = compileAll(file.getFullText(), { fileName })
  const fixes: tsModule.CodeFixAction[] = []
  ...
  for (const e of errors) {
    ...
    if (end < nStart || start > nEnd) continue

    const change = (span: tsModule.TextSpan, newText: string, description: string) => {
      fixes.push({
    ...
    switch (e.code) {
      case 'ST1101': {
        ...
        if (m) {
          change(
            { start: nStart, length: nEnd - nStart },
            `${m[1]}${m[2]} = ${m[2]} ${m[3]} ${m[4]}`,
            `Rewrite as \`${m[2]} = ${m[2]} ${m[3]} …\``,
          )
        }
        break
      }

...
import { runCorpus, type TargetResult } from './corpus.js'
...
const line = (r: TargetResult): string => {
  const flags: string[] = []
  if (!r.rawFree) flags.push('uses raw()')
  if (r.provenance === 'drifted') flags.push('reference drifted from clone')
  if (r.provenance === 'missing') flags.push('clone file missing')
  if (r.meta.adapted) flags.push('adapted reference')
  const suffix = flags.length ? `  [${flags.join('; ')}]` : ''

  if (r.compileError) return `FAIL  ${r.meta.name}\n  compile error: ${r.compileError}${suffix}`

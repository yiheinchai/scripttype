/**
 * ORIGINAL TypeScript from 01-type-level-programming/ts-pattern/src/types/FindSelected.ts, for comparison with the ScriptType alongside.
 *
...
export type MapOptional<selections> = {
  [k in keyof selections]: selections[k] extends [infer v, infer subpath]
    ? [v | undefined, subpath]
    : never;
};

export type MapList<selections> = {
  [k in keyof selections]: selections[k] extends [infer v, infer subpath]
    ? [v[], subpath]
    : never;
};
...
export type ReduceFindSelectionUnion<
  i,

...
/* compiles to:
 * export type MapOptional__v1<selections, k> =
 *   selections[k] extends [infer v, infer subpath] ? [v | undefined, subpath] : never
 * export type MapOptional<selections> = {
 *   [K in keyof selections]: MapOptional__v1<selections, K>
 * }
 */
...
/* compiles to:
 * export type MapList__v1<selections, k> =
 *   selections[k] extends [infer v, infer subpath] ? [v[], subpath] : never
 * export type MapList<selections> = { [K in keyof selections]: MapList__v1<selections, K> }
 */
...
/* compiles to:
 * export type ReduceFindSelectionUnion<i, ps extends readonly any[]> = ReduceFindSelectionUnion__loop<
 *   ps,
 *   never,
 *   i
 * >
 * type ReduceFindSelectionUnion__loop<Ps_ extends readonly any[], Output, I> =
 *   Ps_ extends readonly [infer head, ...infer tail]
 *     ? ReduceFindSelectionUnion__loop<tail, Output | FindSelectionUnion<I, head>, I>
 *     : Output
 */
...
/* compiles to:
 * export type FindSelectionUnionInArray<i, p, path extends any[] = [], output = never> =
 *   i extends readonly (infer iItem)[]
 *     ? p extends readonly [] ? output
 ...
 *     : | output
 *     | FindSelectionUnion<iItem, ValueOf<p>, [...path, Extract<p, readonly any[]>['length']]>
 *     : output
 */
  ...
  if (matches<{}>(p)) {
    if (matches<{}>(i)) {
      ...
      for (const k in keyof(p)) {
        out3[k] = k in i ? FindSelectionUnion(i[k], p[k], [...path, k]) : never
      }
      return out3[keyof(p)]
    }
    return never
...
/* compiles to:
 * export type FindSelectionUnion<i, p, path extends any[] = []> =
 *   0 extends 1 & i ? never
 *   : 0 extends 1 & p ? never
 ...
 *         : never
 *       record: [i, pattern] extends [Record<infer k, infer v>, [infer pkey, infer pvalue]]
 *         ? | MapList<FindSelectionUnion<k, pkey, path>>
 *         | MapList<FindSelectionUnion<v, pvalue, path>>
 *         : never
 *       map: [i, pattern] extends [Map<infer k, infer v>, [infer pkey, infer pvalue]]
 *         ? | MapList<FindSelectionUnion<k, pkey, path>>
 *         | MapList<FindSelectionUnion<v, pvalue, path>>
 *         : never
 ...
 *       not: never
 *       default: sel extends Some<infer k> ? { [Kk1 in k]: [i, path] } : never
 *       custom: never
 *     }[matcherType]
...
/* compiles to:
 * export type Selections<i, p> =
 *   FindSelectionUnion<i, p> extends infer u
 *     ? [u] extends [never] ? i : SelectionToArgs<Extract<MergeUnion<u>, SelectionsRecord>>
 *     : i
 */

...
type pk<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any, T9 = any, T10 = any, T11 = any, T12 = any, T13 = any, T14 = any, T15 = any, T16 = any> = any
type pv<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any, T9 = any, T10 = any, T11 = any, T12 = any, T13 = any, T14 = any, T15 = any, T16 = any> = any
 ...
 * type ReduceUnion__loop<Tuple_ extends readonly any[], Output, I> =
 *   Tuple_ extends readonly [infer p, ...infer tail]
 *     ? ReduceUnion__loop<tail, Output | InvertPatternInternal<p, I>, I>
 *     : Output
 */
...
/* compiles to:
 * export type ReduceIntersection<tuple extends readonly any[], i> = ReduceIntersection__loop<
 *   tuple,
 *   unknown,
 *   i
 * >
 * type ReduceIntersection__loop<Tuple_ extends readonly any[], Output, I> =
 *   Tuple_ extends readonly [infer p, ...infer tail]
 *     ? ReduceIntersection__loop<tail, Output & InvertPatternInternal<p, I>, I>
 *     : Output
 */
...
/* compiles to:
 * export type InvertPattern<p, input> =
 *   Equal<Pattern<input>, p> extends true ? never : InvertPatternInternal<p, input>
 */
...
export function InvertPatternInternal(p, input) {
  ...
  if (matches<true>(IsPlainObject(p))) {
    ...
    if (m4) {
      ...
      for (const k in keySet(Exclude(keyof(p), m4.optKeys))) {
        out2[k] = InvertPatternInternal(p[k], WithDefault(GetKey(ExtractPlainObject(input), k), unknown))
      }
 ...
 *         : Compute<
 *           & {
 *               [K1 in Exclude<keyof p, optKeys>]: InvertPatternInternal<
 *                 p[K1],
 *                 WithDefault<GetKey<ExtractPlainObject<input>, K1>, unknown>
 *               >
 *             }
 *           & {
 *               [K2 in Extract<optKeys, keyof p>]?: InvertPatternInternal<
 *                 p[K2],
 *                 WithDefault<GetKey<ExtractPlainObject<input>, K2>, unknown>
 *               >
 *             }
 *         >
 *       : never
...
/* compiles to:
 * export type ReduceIntersectionForExclude<tuple extends readonly any[], i> = ReduceIntersectionForExclude__loop<
 *   tuple,
 *   unknown,
 *   i
 * >
 * type ReduceIntersectionForExclude__loop<Tuple_ extends readonly any[], Output, I> =
 *   Tuple_ extends readonly [infer p, ...infer tail]
 *     ? ReduceIntersectionForExclude__loop<
 *       tail,
...
/* compiles to:
 * export type ReduceUnionForExclude<tuple extends readonly any[], i> = ReduceUnionForExclude__loop<
 *   tuple,
 *   never,
 *   i
 * >
 * type ReduceUnionForExclude__loop<Tuple_ extends readonly any[], Output, I> =
 *   Tuple_ extends readonly [infer p, ...infer tail]
 *     ? ReduceUnionForExclude__loop<
 *       tail,
 ...
 *   : All<[Extends<a, NonLiteralPrimitive>, Not<IsLiteral<a>>, IsLiteral<b>]> extends true ? never
 *   : DeepExclude<a, b>
 */
 ...
 * > =
 *   i extends readonly (infer ii)[]
 *     ? p extends readonly [] ? MaybeAddReadonly<[...startOutput, ...endOutput], isReadonly>
 *     : p extends readonly [infer p1, ...infer pRest]
 *       ? i extends readonly [infer i1, ...infer iRest]
...
/* compiles to:
 * export type InvertPatternForExclude<p, i> =
 *   Equal<Pattern<i>, p> extends true ? never : InvertPatternForExcludeInternal<p, i>
 */
    ...
    if (matches<object>(i)) {
      ...
      if (m9) {
        if (matches<[ never ]>([m9.optKeys])) {
          const out = emptyObject
          for (const k in keyof(p)) {
            out[k] = readonlyProp(k in i ? InvertPatternForExcludeInternal(p[k], i[k], empty) : InvertPatternInternal(p[k], unknown))
          }
          return out
        }
        const out2 = emptyObject
        for (const k in keySet(Exclude(keyof(p), m9.optKeys))) {
          out2[k] = readonlyProp(k in i ? InvertPatternForExcludeInternal(p[k], i[k], empty) : InvertPatternInternal(p[k], unknown))
        }
        const out3 = emptyObject
 ...
 *       record: subpattern extends [infer pk, infer pv]
 *         ? Record<
 *           Extract<InvertPatternForExcludeInternal<pk, RecordKey<i>, empty>, PropertyKey>,
 ...
 *     : i extends object
 *       ? [keyof p & keyof i] extends [never] ? empty
 *       : OptionalKeys<p> extends infer optKeys
 *         ? [optKeys] extends [never]
 *           ? {
 ...
 *           : Compute<
 *             & {
 *                 readonly [K1 in Exclude<keyof p, optKeys>]: K1 extends keyof i
 *                   ? InvertPatternForExcludeInternal<p[K1], i[K1], empty>
 *                   : InvertPatternInternal<p[K1], unknown>
 *               }
 *             & {
 *                 readonly [K2 in Extract<optKeys, keyof p>]?: K2 extends keyof i
 *                   ? InvertPatternForExcludeInternal<p[K2], i[K2], empty>
 *                   : InvertPatternInternal<p[K2], unknown>
 *               }
 *           >
 *       : empty
 *     : empty

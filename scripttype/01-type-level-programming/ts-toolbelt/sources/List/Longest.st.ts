/**
 * ScriptType for 01-type-level-programming/ts-toolbelt/sources/List/Longest.ts
 *
...
export function Longest(L: List, L1: List): any {
  if (matches<unknown>(L)) {
    if (matches<unknown>(L1)) {
      return { 0: L1, 1: L }[Has(keyof(L), keyof(L1))]
    }
    return never
  }
  return never

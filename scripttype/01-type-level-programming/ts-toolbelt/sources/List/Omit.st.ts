...
export function _Omit(L: List, K: Key): any {
  return _ListOf(_OOmit(ObjectOf(L), anyOf(`${merge(K, number)}`, K)))
}
/* compiles to:
 * export type _Omit<L extends List, K extends Key> = _ListOf<
 *   _OOmit<ObjectOf<L>, `${K & number}` | K>
 * >
 */

// ✓ Omit: verified type-identical to the original
/* @scripttype preserveParamNames */
export function Omit(L: List, K: Key): any {

...
// ✗ _CamelCasedPropertiesDeep: the ScriptType does not itself typecheck as TypeScript
//   _CamelCasedPropertiesDeep.st.ts(11:12) TS2348: Value of type 'SetConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function _CamelCasedPropertiesDeep(Value, Options: Required<CamelCaseOptions>) {
  ...
  if (m1) {
    return Set(_CamelCasedPropertiesDeep(m1.U, Options))
  }
...
export function CamelCasedPropertiesArrayDeep(Value: UnknownArray, Options: Required<CamelCaseOptions>) {
  ...
  if (m4) {
    return Array(_CamelCasedPropertiesDeep(m4.U, Options))
  }
  const m5 = matches<ReadonlyArray<Hole<"U">>>(Value)
  if (m5) {
    return ReadonlyArray(_CamelCasedPropertiesDeep(m5.U, Options))
  }
  return never
}
/* compiles to:

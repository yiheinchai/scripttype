 ...
 */
export function BaseKeyFilter(Type, Key): any {
...
// ✓ FilterOptionalKeys: verified type-identical to the original
/* @scripttype preserveParamNames */
/**
 * @param {object} T
 */
export function FilterOptionalKeys(T): any {
  const out = emptyObject
  for (const Key in keyof(T)) {
...
/* @scripttype preserveParamNames */
export function RequireNone(KeysType: PropertyKey): any {
...
/* @scripttype preserveParamNames */
export function LiteralKeyOf(T): any {
  const out = emptyObject
...
/* @scripttype preserveParamNames */
export function ExactKey(T: object, Key: PropertyKey): any {
  if (Key in T) {
    return Key
  }

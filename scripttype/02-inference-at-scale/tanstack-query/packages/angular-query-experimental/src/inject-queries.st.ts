...
export function QueriesOptions(T: Array<any>, TResults: Array<any> = [], TDepth: ReadonlyArray<number> = []) {
  if (matches<MAXIMUM_DEPTH>(TDepth['length'])) {
    return Array(QueryObserverOptionsForCreateQueries)
  }
  if (matches<[ ]>(T)) {
    return []
  }
  ...
  if (m3) {
    return Array(QueryObserverOptionsForCreateQueries(m3.TQueryFnData, m3.TError, m3.TData, m3.TQueryKey))
  }
  return Array(QueryObserverOptionsForCreateQueries)
}
/* compiles to:
...
export function QueriesResults(T: Array<any>, TResults: Array<any> = [], TDepth: ReadonlyArray<number> = []) {
  if (matches<MAXIMUM_DEPTH>(TDepth['length'])) {
    return Array(CreateQueryResult)
  }
  if (matches<[ ]>(T)) {
    return []
  }

...
// ✗ BaseStreamedQueryParams: the ScriptType does not itself typecheck as TypeScript
//   BaseStreamedQueryParams.st.ts(3:64) TS2693: 'AsyncIterable' only refers to a type, but is being used as a value here.
/* @scripttype preserveParamNames */
export function BaseStreamedQueryParams(TQueryFnData, TQueryKey: QueryKey) {
  return { streamFn: fnType([QueryFunctionContext(TQueryKey)], AsyncIterable(TQueryFnData) | Promise(AsyncIterable(TQueryFnData))), refetchMode: optional(anyOf('append', 'reset', 'replace')) }
}
/* compiles to:

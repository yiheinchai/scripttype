 ...
 * 16 type-level aliases: 9 covered, 6 typecheck-error, 1 compile-error
 *
...
// ✗ ValidateCompositeComponentPromiseResult: the ScriptType does not itself typecheck as TypeScript
//   ValidateCompositeComponentPromiseResult.st.ts(5:12) TS2348: Value of type 'PromiseConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function ValidateCompositeComponentPromiseResult(TPromise) {
  const m1 = matches<Promise<Hole<"T">>>(TPromise)
  if (m1) {
    return Promise(ValidateCompositeComponentResult(m1.T))
  }
  return never
}
/* compiles to:

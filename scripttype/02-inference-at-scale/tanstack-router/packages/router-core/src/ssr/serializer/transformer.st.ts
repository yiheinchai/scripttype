 ...
 * 15 type-level aliases: 11 covered, 4 typecheck-error
 *
...
// ✗ ValidateSerializableAsyncGenerator: the ScriptType does not itself typecheck as TypeScript
//   ValidateSerializableAsyncGenerator.st.ts(5:12) TS2693: 'AsyncGenerator' only refers to a type, but is being used as a value here.
/* @scripttype preserveParamNames */
export function ValidateSerializableAsyncGenerator(T, TSerializable) {
  ...
  if (m1) {
    return AsyncGenerator(ValidateSerializable(m1.T, TSerializable), ValidateSerializable(m1.TReturn, TSerializable), m1.TNext)
  }
  return never
}
/* compiles to:
...
// ✗ ValidateSerializablePromise: the ScriptType does not itself typecheck as TypeScript
//   ValidateSerializablePromise.st.ts(5:12) TS2348: Value of type 'PromiseConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function ValidateSerializablePromise(T, TSerializable) {
  const m1 = matches<Promise<Hole<"TAwaited">>>(T)
  if (m1) {
    return Promise(ValidateSerializable(m1.TAwaited, TSerializable))
  }
  return never
}
/* compiles to:
...
// ✗ ValidateSerializableSet: the ScriptType does not itself typecheck as TypeScript
//   ValidateSerializableSet.st.ts(5:12) TS2348: Value of type 'SetConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function ValidateSerializableSet(T, TSerializable) {
  const m1 = matches<Set<Hole<"TItem">>>(T)
  if (m1) {
    return Set(ValidateSerializable(m1.TItem, TSerializable))
  }
  return never
}
/* compiles to:
...
// ✗ ValidateSerializableMap: the ScriptType does not itself typecheck as TypeScript
//   ValidateSerializableMap.st.ts(5:12) TS2348: Value of type 'MapConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function ValidateSerializableMap(T, TSerializable) {
  const m1 = matches<Map<Hole<"TKey">, Hole<"TValue">>>(T)
  if (m1) {
    return Map(ValidateSerializable(m1.TKey, TSerializable), ValidateSerializable(m1.TValue, TSerializable))
  }
  return never
}
/* compiles to:
...
export function ValidateSerializableArray(T, TSerializable) {
  ...
  if (m1) {
    return Array(ValidateSerializable(m1.U, TSerializable))
  }
  const m2 = matches<ReadonlyArray<Hole<"U">>>(T)
  if (m2) {
    return ReadonlyArray(ValidateSerializable(m2.U, TSerializable))
  }
  return never
}
/* compiles to:

...
// ✗ HTTPBatchLinkOptions: the ScriptType does not itself typecheck as TypeScript
//   HTTPBatchLinkOptions.st.ts(3:171) TS2348: Value of type 'PromiseConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function HTTPBatchLinkOptions(TRoot: AnyClientTypes) {
  return merge(HTTPLinkBaseOptions(TRoot), { maxURLLength: optional(number), headers: optional(HTTPHeaders | fnType([{ opList: NonEmptyArray(Operation) }], HTTPHeaders | Promise(HTTPHeaders))), maxItems: optional(number) })
}
/* compiles to:

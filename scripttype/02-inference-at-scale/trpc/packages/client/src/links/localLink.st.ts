...
// ✗ LocalLinkOptions: the ScriptType does not itself typecheck as TypeScript
//   LocalLinkOptions.st.ts(3:61) TS2348: Value of type 'PromiseConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function LocalLinkOptions(TRouter: AnyRouter) {
  return merge({ router: TRouter, createContext: fnType([], Promise(inferRouterContext(TRouter))), onError: optional(fnType([ErrorHandlerOptions(inferRouterContext(TRouter))], voidType())) }, TransformerOptions(inferClientTypes(TRouter)))
}
/* compiles to:

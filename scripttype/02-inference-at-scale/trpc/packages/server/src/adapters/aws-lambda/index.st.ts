...
// ✗ AWSLambdaCreateContextFn: the ScriptType does not itself typecheck as TypeScript
//   AWSLambdaCreateContextFn.st.ts(3:88) TS2348: Value of type 'PromiseConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function AWSLambdaCreateContextFn(TRouter: AnyRouter, TEvent: LambdaEvent) {
  return fnType([CreateAWSLambdaContextOptions(TEvent)], inferRouterContext(TRouter) | Promise(inferRouterContext(TRouter)))
}
/* compiles to:

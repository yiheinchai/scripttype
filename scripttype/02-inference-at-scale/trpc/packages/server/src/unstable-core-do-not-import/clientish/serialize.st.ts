...
// ✗ Serialize: the ScriptType does not itself typecheck as TypeScript
//   Serialize.st.ts(14:12) TS2693: 'AsyncIterable' only refers to a type, but is being used as a value here.
/* @scripttype preserveParamNames */
export function Serialize(T) {
  ...
  if (m1) {
    return AsyncIterable(Serialize(m1.$T), Serialize(m1.$Return), Serialize(m1.$Next))
  }
  const m2 = matches<PromiseLike<Hole<"$T">>>(T)
  if (m2) {
    return Promise(Serialize(m2.$T))
  }

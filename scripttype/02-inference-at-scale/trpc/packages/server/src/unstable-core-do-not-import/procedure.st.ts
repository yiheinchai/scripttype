/**
 * ScriptType for 02-inference-at-scale/trpc/packages/server/src/unstable-core-do-not-import/procedure.ts
 *
 * Generated by the ScriptType decompiler from the corpus source above, then verified
 * by recompiling and comparing to the original with the TypeScript checker.
 * 5 type-level aliases: 5 covered
 *
 * Do not edit: regenerate with `tsx src/materialize.ts`.
 */
// Types this file references but does not define. Those whose declaration could be
// found are imported, so the check is against the real type rather than `any`; the
// rest are stubbed. Each also gets a value declaration, because ScriptType applies
// types in call position and a type-only import binds nothing in value space.
import type { AnyProcedure, AnySubscriptionProcedure, LegacyObservableSubscriptionProcedure } from '../../../../../../../02-inference-at-scale/trpc/packages/server/src/unstable-core-do-not-import/procedure.js'
import type { inferAsyncIterableYield } from '../../../../../../../02-inference-at-scale/trpc/packages/server/src/unstable-core-do-not-import/types.js'
declare const AnyProcedure: any
declare const AnySubscriptionProcedure: any
declare const LegacyObservableSubscriptionProcedure: any
declare const inferAsyncIterableYield: any
type inferProcedureParams<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any, T9 = any, T10 = any, T11 = any, T12 = any, T13 = any, T14 = any, T15 = any, T16 = any> = any
// ✓ inferProcedureInput: verified type-identical to the original
/* @scripttype preserveParamNames */
export function inferProcedureInput(TProcedure: AnyProcedure): any {
  if (matches<inferProcedureParams<typeof TProcedure>['$types']['input']>(Undefined)) {
    return voidType() | inferProcedureParams(TProcedure)['$types']['input']
  }
  return inferProcedureParams(TProcedure)['$types']['input']
}
/* compiles to:
 * export type inferProcedureInput<TProcedure extends AnyProcedure> =
 *   undefined extends inferProcedureParams<TProcedure>['$types']['input']
 *     ? void | inferProcedureParams<TProcedure>['$types']['input']
 *     : inferProcedureParams<TProcedure>['$types']['input']
 */

// ✓ inferProcedureParams: verified type-identical to the original
/* @scripttype preserveParamNames */
export function inferProcedureParams(TProcedure): any {
  if (matches<AnyProcedure>(TProcedure)) {
    return TProcedure['_def']
  }
  return never
}
/* compiles to:
 * export type inferProcedureParams<TProcedure> =
 *   TProcedure extends AnyProcedure ? TProcedure['_def'] : never
 */

// ✓ inferProcedureOutput: verified type-identical to the original
/* @scripttype preserveParamNames */
export function inferProcedureOutput(TProcedure): any {
  return inferProcedureParams(TProcedure)['$types']['output']
}
/* compiles to:
 * export type inferProcedureOutput<TProcedure> = inferProcedureParams<TProcedure>['$types']['output']
 */

// ✓ inferSubscriptionInput: verified type-identical to the original
/* @scripttype preserveParamNames */
export function inferSubscriptionInput(TProcedure: AnySubscriptionProcedure): any {
  return inferProcedureInput(TProcedure)
}
/* compiles to:
 * export type inferSubscriptionInput<TProcedure extends AnySubscriptionProcedure> = inferProcedureInput<
 *   TProcedure
 * >
 */

// ✓ inferSubscriptionOutput: verified type-identical to the original
/* @scripttype preserveParamNames */
export function inferSubscriptionOutput(TProcedure: AnySubscriptionProcedure): any {
  if (matches<LegacyObservableSubscriptionProcedure<any>>(TProcedure)) {
    return inferProcedureOutput(TProcedure)
  }
  return inferAsyncIterableYield(inferProcedureOutput(TProcedure))
}
/* compiles to:
 * export type inferSubscriptionOutput<TProcedure extends AnySubscriptionProcedure> =
 *   TProcedure extends LegacyObservableSubscriptionProcedure<any>
 *     ? inferProcedureOutput<TProcedure>
 *     : inferAsyncIterableYield<inferProcedureOutput<TProcedure>>
 */

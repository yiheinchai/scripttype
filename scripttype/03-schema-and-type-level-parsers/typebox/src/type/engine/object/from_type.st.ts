...
export function TFromType__v1(Type): any {
  ...
  const m5 = matches<TTuple<Hole<"Types", TSchema[]>>>(Type)
  if (m5) {
    return TFromTuple(m5.Types)
  }
  const m6 = matches<TObject<Hole<"Properties", TProperties>>>(Type)
  if (m6) {
    return TFromObject(m6.Properties)
  }
  return {}
}
export function TFromType(Type: TSchema, Result: TProperties = TFromType__v1(Type)): any {
  return Result
}
/* compiles to:
 ...
 *   : {}
 * export type TFromType<Type extends TSchema, Result extends TProperties = TFromType__v1<Type>> = Result
 */

/**
 * ScriptType for 03-schema-and-type-level-parsers/typebox/src/type/extends/any.ts
 *
 * Generated by the ScriptType decompiler from the corpus source above, then verified
 * by recompiling and comparing to the original with the TypeScript checker.
 * 1 type-level aliases: 1 covered
 *
 * Do not edit: regenerate with `tsx src/materialize.ts`.
 */
// Types this file references but does not define. Those whose declaration could be
// found are imported, so the check is against the real type rather than `any`; the
// rest are stubbed. Each also gets a value declaration, because ScriptType applies
// types in call position and a type-only import binds nothing in value space.
import type { TExtendsRight } from '../../../../../../03-schema-and-type-level-parsers/typebox/src/type/extends/extends_right.js'
import type { TExtendsTrue } from '../../../../../../03-schema-and-type-level-parsers/typebox/src/type/extends/result.js'
import type { TAny } from '../../../../../../03-schema-and-type-level-parsers/typebox/src/type/types/any.js'
import type { TInfer } from '../../../../../../03-schema-and-type-level-parsers/typebox/src/type/types/infer.js'
import type { TSchema } from '../../../../../../03-schema-and-type-level-parsers/typebox/src/type/types/schema.js'
import type { TUnknown } from '../../../../../../03-schema-and-type-level-parsers/typebox/src/type/types/unknown.js'
declare namespace Result {
  export type TExtendsTrue<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any, T9 = any, T10 = any, T11 = any, T12 = any, T13 = any, T14 = any, T15 = any, T16 = any> = any
  export type TExtendsUnion<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any, T9 = any, T10 = any, T11 = any, T12 = any, T13 = any, T14 = any, T15 = any, T16 = any> = any
}
declare const Result: any
declare const TAny: any
declare const TExtendsRight: any
declare const TExtendsTrue: any
declare const TExtendsUnion: any
declare const TInfer: any
declare const TProperties: any
declare const TSchema: any
declare const TUnknown: any
type Result<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any, T9 = any, T10 = any, T11 = any, T12 = any, T13 = any, T14 = any, T15 = any, T16 = any> = any
type TExtendsUnion<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any, T9 = any, T10 = any, T11 = any, T12 = any, T13 = any, T14 = any, T15 = any, T16 = any> = any
type TProperties<T1 = any, T2 = any, T3 = any, T4 = any, T5 = any, T6 = any, T7 = any, T8 = any, T9 = any, T10 = any, T11 = any, T12 = any, T13 = any, T14 = any, T15 = any, T16 = any> = any
// ✓ TExtendsAny: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TExtendsAny(Inferred: TProperties, Left: TAny, Right: TSchema): any {
  if (matches<TInfer>(Right)) {
    return TExtendsRight(Inferred, Left, Right)
  }
  if (matches<TAny>(Right)) {
    return t<Result.TExtendsTrue<typeof Inferred>>()
  }
  if (matches<TUnknown>(Right)) {
    return t<Result.TExtendsTrue<typeof Inferred>>()
  }
  return t<Result.TExtendsUnion<typeof Inferred>>()
}
/* compiles to:
 * export type TExtendsAny<
 *   Inferred extends TProperties,
 *   Left extends TAny,
 *   Right extends TSchema
 * > =
 *   Right extends TInfer ? TExtendsRight<Inferred, Left, Right>
 *   : Right extends TAny ? Result.TExtendsTrue<Inferred>
 *   : Right extends TUnknown ? Result.TExtendsTrue<Inferred>
 *   : Result.TExtendsUnion<Inferred>
 */

/**
 * ScriptType for 03-schema-and-type-level-parsers/typebox/src/type/extends/tuple.ts
 *
 * Generated by the ScriptType decompiler from the corpus source above, then verified
 * by recompiling and comparing to the original with the TypeScript checker.
 * 10 type-level aliases: 6 covered, 3 typecheck-error, 1 compile-error
 *
 * Do not edit: regenerate with `tsx src/materialize.ts`.
 */
// Names this file references but does not define: types from elsewhere in the
// library, and local functions used in type position. Declared so the generated
// ScriptType typechecks standalone. They carry no runtime meaning.
declare const TArray: any
declare const TExtendsLeft: any
declare const TExtendsRight: any
declare const TExtendsTrueLike: any
declare const TInferTupleResult: any
declare const TInferUnionResult: any
declare const TInferable: any
declare const TInstantiateElements: any
declare const TProperties: any
declare const TSchema: any
declare const TState: any
declare const TTryInferable: any
declare const TTryRestInferable: any
declare const TTuple: any
type TArray<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TExtendsLeft<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TExtendsRight<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TExtendsTrueLike<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TInferTupleResult<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TInferUnionResult<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TInferable<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TInstantiateElements<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TProperties<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TSchema<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TState<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TTryInferable<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TTryRestInferable<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TTuple<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
// ✗ TReverse: the ScriptType does not itself typecheck as TypeScript
//   TReverse.st.ts(11:20) TS2345: Argument of type 'any' is not assignable to parameter of type 'never'.
/* @scripttype preserveParamNames */
export function TReverse(Types: TSchema[]) {
  let Result = []
  let types = Types
  while (true) {
    const m1 = matches<[ Hole<"Left", TSchema>, ...Hole<"Right", TSchema[]> ]>(types)
    if (!m1) {
      break
    }
    types = m1.Right
    Result.unshift(m1.Left)
  }
  return Result
}
/* compiles to:
 * export type TReverse<Types extends TSchema[]> = TReverse__loop<Types, []>
 * type TReverse__loop<Types extends TSchema[], Result extends readonly unknown[]> = Types extends [infer Left extends TSchema, ...(infer Right extends TSchema[])] ? TReverse__loop<Right, [Left, ...Result]> : Result
 */

// ✓ TApplyReverse: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TApplyReverse(Types: TSchema[], Reversed: boolean) {
  if (matches<true>(Reversed)) {
    return TReverse(Types)
  }
  return Types
}
/* compiles to:
 * export type TApplyReverse<Types extends TSchema[], Reversed extends boolean> = Reversed extends true ? TReverse<Types> : Types
 */

// ✗ TReversed: does not compile yet
//   Cannot find name 'm1'.
/* @scripttype preserveParamNames */
export function TReversed(Types: TSchema[], First: TSchema | undefined = m1 ? m1.Left : Undefined, Inferable: TSchema | undefined = matches<TSchema>(First) ? TTryRestInferable(First) : Undefined, Result: boolean = matches<TSchema>(Inferable) ? true : false) {
  return Result
}
/* compiles to:
 * export type TReversed<Types extends TSchema[], First extends TSchema | undefined = m1 extends true ? m1.Left : undefined, Inferable extends TSchema | undefined = First extends TSchema ? TTryRestInferable<First> : undefined, Result extends boolean = Inferable extends TSchema ? true : false> = Result
 */

// ✗ TElementsCompare: the ScriptType does not itself typecheck as TypeScript
//   TElementsCompare.st.ts(3:22) TS2702: 'Result' only refers to a type, but is being used as a namespace here.
/* @scripttype preserveParamNames */
export function TElementsCompare(Inferred: TProperties, Reversed: boolean, Left: TSchema, LeftRest: TSchema[], Right: TSchema, RightRest: TSchema[]) {
  const m1 = matches<Result.TExtendsTrueLike<Hole<"CheckInferred", TProperties>>>(TExtendsLeft(Inferred, Left, Right))
  if (m1) {
    return TElements(m1.CheckInferred, Reversed, LeftRest, RightRest)
  }
  return Result.TExtendsFalse
}
/* compiles to:
 * export type TElementsCompare<Inferred extends TProperties, Reversed extends boolean, Left extends TSchema, LeftRest extends TSchema[], Right extends TSchema, RightRest extends TSchema[]> = TExtendsLeft<Inferred, Left, Right> extends Result.TExtendsTrueLike<infer CheckInferred extends TProperties> ? TElements<CheckInferred, Reversed, LeftRest, RightRest> : Result.TExtendsFalse
 */

// ✓ TElementsLeft: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TElementsLeft(Inferred: TProperties, Reversed: boolean, LeftRest: TSchema[], Right: TSchema, RightRest: TSchema[], Inferable: TInferable | undefined = TTryRestInferable(Right)) {
  if (matches<TInferable>(Inferable)) {
    return TInferTupleResult(Inferred, Inferable['name'], TApplyReverse(LeftRest, Reversed), Inferable['type'])
  }
  const m1 = matches<[ Hole<"Head", TSchema>, ...Hole<"Tail", TSchema[]> ]>(LeftRest)
  if (m1) {
    return TElementsCompare(Inferred, Reversed, m1.Head, m1.Tail, Right, RightRest)
  }
  return Result.TExtendsFalse
}
/* compiles to:
 * export type TElementsLeft<Inferred extends TProperties, Reversed extends boolean, LeftRest extends TSchema[], Right extends TSchema, RightRest extends TSchema[], Inferable extends TInferable | undefined = TTryRestInferable<Right>> = Inferable extends TInferable ? TInferTupleResult<Inferred, Inferable['name'], TApplyReverse<LeftRest, Reversed>, Inferable['type']> : LeftRest extends [infer Head extends TSchema, ...(infer Tail extends TSchema[])] ? TElementsCompare<Inferred, Reversed, Head, Tail, Right, RightRest> : Result.TExtendsFalse
 */

// ✓ TElementsRight: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TElementsRight(Inferred: TProperties, Reversed: boolean, LeftRest: TSchema[], RightRest: TSchema[]) {
  const m1 = matches<[ Hole<"Head", TSchema>, ...Hole<"Tail", TSchema[]> ]>(RightRest)
  if (m1) {
    return TElementsLeft(Inferred, Reversed, LeftRest, m1.Head, m1.Tail)
  }
  if (matches<0>(LeftRest['length'])) {
    return Result.TExtendsTrue(Inferred)
  }
  return Result.TExtendsFalse
}
/* compiles to:
 * export type TElementsRight<Inferred extends TProperties, Reversed extends boolean, LeftRest extends TSchema[], RightRest extends TSchema[]> = RightRest extends [infer Head extends TSchema, ...(infer Tail extends TSchema[])] ? TElementsLeft<Inferred, Reversed, LeftRest, Head, Tail> : LeftRest['length'] extends 0 ? Result.TExtendsTrue<Inferred> : Result.TExtendsFalse
 */

// ✓ TElements: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TElements(Inferred: TProperties, Reversed: boolean, LeftRest: TSchema[], RightRest: TSchema[]) {
  return TElementsRight(Inferred, Reversed, LeftRest, RightRest)
}
/* compiles to:
 * export type TElements<Inferred extends TProperties, Reversed extends boolean, LeftRest extends TSchema[], RightRest extends TSchema[]> = TElementsRight<Inferred, Reversed, LeftRest, RightRest>
 */

// ✓ TExtendsTupleToTuple: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TExtendsTupleToTuple(Inferred: TProperties, Left: TSchema[], Right: TSchema[], InstantiatedRight: TSchema[] = TInstantiateElements(Inferred, TState([], []), Right), Reversed: boolean = TReversed(InstantiatedRight)) {
  return TElements(Inferred, Reversed, TApplyReverse(Left, Reversed), TApplyReverse(InstantiatedRight, Reversed))
}
/* compiles to:
 * export type TExtendsTupleToTuple<Inferred extends TProperties, Left extends TSchema[], Right extends TSchema[], InstantiatedRight extends TSchema[] = TInstantiateElements<Inferred, TState<[], []>, Right>, Reversed extends boolean = TReversed<InstantiatedRight>> = TElements<Inferred, Reversed, TApplyReverse<Left, Reversed>, TApplyReverse<InstantiatedRight, Reversed>>
 */

// ✗ TExtendsTupleToArray: the ScriptType does not itself typecheck as TypeScript
//   TExtendsTupleToArray.st.ts(8:24) TS2702: 'Result' only refers to a type, but is being used as a namespace here.
/* @scripttype preserveParamNames */
export function TExtendsTupleToArray(Inferred: TProperties, Left: TSchema[], Right: TSchema, Inferrable: TInferable | undefined = TTryInferable(Right)) {
  if (matches<TInferable>(Inferrable)) {
    return TInferUnionResult(Inferred, Inferrable['name'], Left, Inferrable['type'])
  }
  const m1 = matches<[ Hole<"Head", TSchema>, ...Hole<"Tail", TSchema[]> ]>(Left)
  if (m1) {
    const m2 = matches<Result.TExtendsTrueLike<Hole<"Inferred", TProperties>>>(TExtendsLeft(Inferred, m1.Head, Right))
    if (m2) {
      return TExtendsTupleToArray(m2.Inferred, m1.Tail, Right)
    }
    return Result.TExtendsFalse
  }
  return Result.TExtendsTrue(Inferred)
}
/* compiles to:
 * export type TExtendsTupleToArray<Inferred extends TProperties, Left extends TSchema[], Right extends TSchema, Inferrable extends TInferable | undefined = TTryInferable<Right>> = Inferrable extends TInferable ? TInferUnionResult<Inferred, Inferrable['name'], Left, Inferrable['type']> : Left extends [infer Head extends TSchema, ...(infer Tail extends TSchema[])] ? TExtendsLeft<Inferred, Head, Right> extends Result.TExtendsTrueLike<infer Inferred extends TProperties> ? TExtendsTupleToArray<Inferred, Tail, Right> : Result.TExtendsFalse : Result.TExtendsTrue<Inferred>
 */

// ✓ TExtendsTuple: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TExtendsTuple(Inferred: TProperties, Left: TSchema[], Right: TSchema, InstantiatedLeft: TSchema[] = TInstantiateElements(Inferred, TState([], []), Left)) {
  const m1 = matches<TTuple<Hole<"Types", TSchema[]>>>(Right)
  if (m1) {
    return TExtendsTupleToTuple(Inferred, InstantiatedLeft, m1.Types)
  }
  const m2 = matches<TArray<Hole<"Type", TSchema>>>(Right)
  if (m2) {
    return TExtendsTupleToArray(Inferred, InstantiatedLeft, m2.Type)
  }
  return TExtendsRight(Inferred, TTuple(InstantiatedLeft), Right)
}
/* compiles to:
 * export type TExtendsTuple<Inferred extends TProperties, Left extends TSchema[], Right extends TSchema, InstantiatedLeft extends TSchema[] = TInstantiateElements<Inferred, TState<[], []>, Left>> = Right extends TTuple<infer Types extends TSchema[]> ? TExtendsTupleToTuple<Inferred, InstantiatedLeft, Types> : Right extends TArray<infer Type extends TSchema> ? TExtendsTupleToArray<Inferred, InstantiatedLeft, Type> : TExtendsRight<Inferred, TTuple<InstantiatedLeft>, Right>
 */

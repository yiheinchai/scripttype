/**
 * ScriptType for 03-schema-and-type-level-parsers/typebox/src/type/script/token/bigint.ts
 *
 * Generated by the ScriptType decompiler from the corpus source above, then verified
 * by recompiling and comparing to the original with the TypeScript checker.
 * 2 type-level aliases: 2 covered
 *
 * Do not edit: regenerate with `tsx src/materialize.ts`.
 */
// Names this file references but does not define: types from elsewhere in the
// library, and local functions used in type position. Declared so the generated
// ScriptType typechecks standalone. They carry no runtime meaning.
declare const TInteger: any
declare const TTake: any
type TInteger<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type TTake<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
// ✓ TTakeBigInt: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TTakeBigInt(Input: string) {
  const m1 = matches<[ Hole<"Integer", string>, Hole<"IntegerRest", string> ]>(TInteger(Input))
  if (m1) {
    const m2 = matches<[ Hole<"_N", string>, Hole<"NRest", string> ]>(TTake(['n'], m1.IntegerRest))
    if (m2) {
      return [`${m1.Integer}`, m2.NRest]
    }
    return []
  }
  return []
}
/* compiles to:
 * export type TTakeBigInt<Input extends string> = TInteger<Input> extends [infer Integer extends string, infer IntegerRest extends string] ? TTake<['n'], IntegerRest> extends [string, infer NRest extends string] ? [`${Integer}`, NRest] : [] : []
 */

// ✓ TBigInt: verified type-identical to the original
/* @scripttype preserveParamNames */
export function TBigInt(Input: string) {
  return TTakeBigInt(Input)
}
/* compiles to:
 * export type TBigInt<Input extends string> = TTakeBigInt<Input>
 */

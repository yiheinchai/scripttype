...
// ✗ SchemaWithFallbackAsync: the ScriptType does not itself typecheck as TypeScript
//   SchemaWithFallbackAsync.st.ts(3:291) TS2348: Value of type 'PromiseConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function SchemaWithFallbackAsync(TSchema: BaseSchema<unknown, unknown, BaseIssue<unknown>> | BaseSchemaAsync<unknown, unknown, BaseIssue<unknown>>, TFallback: FallbackAsync<typeof TSchema>) {
  return merge(Omit(TSchema, anyOf('async', '~standard', '~run')), { fallback: readonlyProp(TFallback), async: readonlyProp(true), '~standard': readonlyProp(StandardProps(InferInput(TSchema), InferOutput(TSchema))), '~run': readonlyProp(fnType([UnknownDataset, Config(BaseIssue(unknown))], Promise(OutputDataset(InferOutput(TSchema), InferIssue(TSchema))))) })
}
/* compiles to:

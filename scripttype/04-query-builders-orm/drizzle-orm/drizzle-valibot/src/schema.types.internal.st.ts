/**
 * ScriptType for 04-query-builders-orm/drizzle-orm/drizzle-valibot/src/schema.types.internal.ts
 *
 * Generated by the ScriptType decompiler from the corpus source above, then verified
 * by recompiling and comparing to the original with the TypeScript checker.
 * 6 type-level aliases: 4 typecheck-error, 1 covered, 1 raw
 *
 * Do not edit: regenerate with `tsx src/materialize.ts`.
 */
// Names this file references but does not define: types from elsewhere in the
// library, and local functions used in type position. Declared so the generated
// ScriptType typechecks standalone. They carry no runtime meaning.
declare const Assume: any
declare const Column: any
declare const ColumnIsGeneratedAlwaysAs: any
declare const DrizzleTypeError: any
declare const ExtractAdditionalProperties: any
declare const GenericSchema: any
declare const GetSelection: any
declare const GetValibotType: any
declare const HasBaseColumn: any
declare const SelectedFieldsFlat: any
declare const Simplify: any
declare const Table: any
declare const View: any
declare const v: any
type Assume<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type Column<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type ColumnIsGeneratedAlwaysAs<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type DrizzleTypeError<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type ExtractAdditionalProperties<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type GenericSchema<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type GetSelection<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type GetValibotType<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type HasBaseColumn<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type SelectedFieldsFlat<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type Simplify<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type Table<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type View<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type v<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
// ✗ BuildRefineField: the ScriptType does not itself typecheck as TypeScript
//   BuildRefineField.st.ts(3:15) TS2702: 'v' only refers to a type, but is being used as a namespace here.
/* @scripttype preserveParamNames */
export function BuildRefineField(T) {
  if (matches<v.GenericSchema>(T)) {
    return fnType([T], v.GenericSchema) | v.GenericSchema
  }
  return never
}
/* compiles to:
 * export type BuildRefineField<T> = T extends v.GenericSchema ? ((a0: T) => v.GenericSchema) | v.GenericSchema : never
 */

// ✓ BuildRefine: verified type-identical to the original
/* @scripttype preserveParamNames */
export function BuildRefine(TColumns: Record<string, any>) {
  const out = emptyObject
  for (const K in keyof(TColumns)) {
    out[matches<Column | SelectedFieldsFlat<Column> | Table | View>(TColumns[K]) ? K : never] = optional(matches<Column>(TColumns[K]) ? BuildRefineField(GetValibotType(TColumns[K]['_']['data'], TColumns[K]['_']['dataType'], TColumns[K]['_']['columnType'], TColumns[K]['_']['enumValues'], matches<true>(HasBaseColumn(TColumns[K])) ? Assume(TColumns[K]['_']['baseColumn'], Column) : Undefined, ExtractAdditionalProperties(TColumns[K]))) : BuildRefine(GetSelection(TColumns[K])))
  }
  return out
}
/* compiles to:
 * export type BuildRefine<TColumns extends Record<string, any>> = { [K in keyof TColumns as TColumns[K] extends Column | SelectedFieldsFlat<Column> | Table | View ? K : never]?: TColumns[K] extends Column ? BuildRefineField<GetValibotType<TColumns[K]['_']['data'], TColumns[K]['_']['dataType'], TColumns[K]['_']['columnType'], TColumns[K]['_']['enumValues'], HasBaseColumn<TColumns[K]> extends true ? Assume<TColumns[K]['_']['baseColumn'], Column> : undefined, ExtractAdditionalProperties<TColumns[K]>>> : BuildRefine<GetSelection<TColumns[K]>> }
 */

// ✗ HandleRefinement: the ScriptType does not itself typecheck as TypeScript
//   HandleRefinement.st.ts(3:32) TS2702: 'v' only refers to a type, but is being used as a namespace here.
/* @scripttype preserveParamNames */
export function HandleRefinement(TType: 'select' | 'insert' | 'update', TRefinement, TColumn: Column) {
  if (matches<(schema: any) => v.GenericSchema>(TRefinement)) {
    const m1 = matches<Hole<"TSchema">>(matches<true>(TColumn['_']['notNull']) ? ReturnType(TRefinement) : v.NullableSchema(ReturnType(TRefinement), Undefined))
    if (m1) {
      if (matches<'update'>(TType)) {
        return v.OptionalSchema(Assume(m1.TSchema, v.GenericSchema), Undefined)
      }
      return m1.TSchema
    }
    return v.AnySchema
  }
  return TRefinement
}
/* compiles to:
 * export type HandleRefinement<TType extends 'select' | 'insert' | 'update', TRefinement, TColumn extends Column> = TRefinement extends (schema: any) => v.GenericSchema ? (TColumn['_']['notNull'] extends true ? ReturnType<TRefinement> : v.NullableSchema<ReturnType<TRefinement>, undefined>) extends infer TSchema ? TType extends 'update' ? v.OptionalSchema<Assume<TSchema, v.GenericSchema>, undefined> : TSchema : v.AnySchema : TRefinement
 */

// ✗ IsRefinementDefined: the ScriptType does not itself typecheck as TypeScript
//   IsRefinementDefined.st.ts(4:17) TS2702: 'v' only refers to a type, but is being used as a namespace here.
/* @scripttype preserveParamNames */
export function IsRefinementDefined(TRefinements: Record<string | symbol | number, any> | undefined, TKey: string | symbol | number) {
  if (matches<object>(TRefinements)) {
    if (matches<v.GenericSchema | ((schema: any) => any)>(TRefinements[TKey])) {
      return true
    }
    return false
  }
  return false
}
/* compiles to:
 * export type IsRefinementDefined<TRefinements extends Record<string | symbol | number, any> | undefined, TKey extends string | symbol | number> = TRefinements extends object ? TRefinements[TKey] extends v.GenericSchema | (schema: any) => any ? true : false : false
 */

// ✗ BuildSchema: uses raw() — language gap, does not count as covered
//   gap: pattern bindings inside a mapped-type value
/* @scripttype preserveParamNames */
export function BuildSchema(TType: 'select' | 'insert' | 'update', TColumns: Record<string, any>, TRefinements: Record<string, any> | undefined) {
  const out = emptyObject
  for (const K in keyof(TColumns)) {
    out[matches<true>(ColumnIsGeneratedAlwaysAs(TColumns[K])) ? never : K] = readonlyProp(raw('TColumns[K] extends infer TColumn extends Column ? IsRefinementDefined<TRefinements, Assume<K, string>> extends true ? Assume<HandleRefinement<TType, TRefinements[K & keyof TRefinements], TColumn>, v.GenericSchema> : HandleColumn<TType, TColumn> : TColumns[K] extends infer TObject extends SelectedFieldsFlat<Column> | Table | View ? BuildSchema<TType, GetSelection<TObject>, TRefinements extends object ? TRefinements[K & keyof TRefinements] : undefined> : v.AnySchema'))
  }
  return v.ObjectSchema(Simplify(out), Undefined)
}

// ✗ NoUnknownKeys: the ScriptType does not itself typecheck as TypeScript
//   NoUnknownKeys.st.ts(5:74) TS2702: 'v' only refers to a type, but is being used as a namespace here.
/* @scripttype preserveParamNames */
export function NoUnknownKeys(TRefinement: Record<string, any>, TCompare: Record<string, any>) {
  const out = emptyObject
  for (const K in keyof(TRefinement)) {
    out[K] = matches<keyof typeof TCompare>(K) ? (matches<Record<string, v.GenericSchema>>(TRefinement[K]) ? NoUnknownKeys(TRefinement[K], TCompare[K]) : TRefinement[K]) : DrizzleTypeError(`Found unknown key in refinement: "${merge(K, string)}"`)
  }
  return out
}
/* compiles to:
 * export type NoUnknownKeys<TRefinement extends Record<string, any>, TCompare extends Record<string, any>> = { [K in keyof TRefinement]: K extends keyof TCompare ? TRefinement[K] extends Record<string, v.GenericSchema> ? NoUnknownKeys<TRefinement[K], TCompare[K]> : TRefinement[K] : DrizzleTypeError<`Found unknown key in refinement: "${K & string}"`> }
 */

/**
 * ScriptType for 05-functional-effects-hkt/effect/packages/effect/src/Unify.ts
 *
 * Generated by the ScriptType decompiler from the corpus source above, then verified
 * by recompiling and comparing to the original with the TypeScript checker.
 * 9 type-level aliases: 9 covered
 *
 * Do not edit: regenerate with `tsx src/materialize.ts`.
 */
// Names this file references but does not define: types from elsewhere in the
// library, and local functions used in type position. Declared so the generated
// ScriptType typechecks standalone. They carry no runtime meaning.
declare const ignoreSymbol: any
declare const typeSymbol: any
declare const unifySymbol: any
type ignoreSymbol<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type typeSymbol<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
type unifySymbol<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
// ✓ MaybeReturn: verified type-identical to the original
/* @scripttype preserveParamNames */
export function MaybeReturn(F) {
  const m1 = matches<() => Hole<"R">>(F)
  if (m1) {
    return m1.R
  }
  return NonNullable(F)
}
/* compiles to:
 * export type MaybeReturn<F> = F extends () => infer R ? R : NonNullable<F>
 */

// ✓ Keys: verified type-identical to the original
/* @scripttype preserveParamNames */
export function Keys(X: [ any, any ]) {
  const m1 = matches<[ Hole<"A">, Hole<"Ignore"> ]>(X)
  if (m1) {
    return Exclude(keyof(m1.A), m1.Ignore)
  }
  return never
}
/* compiles to:
 * export type Keys<X extends [any, any]> = X extends [infer A, infer Ignore] ? Exclude<keyof A, Ignore> : never
 */

// ✓ Values: verified type-identical to the original
/* @scripttype preserveParamNames */
export function Values(X: [ any, any ]) {
  const m1 = matches<[ Hole<"A">, Hole<"Ignore"> ]>(X)
  if (m1) {
    const m2 = matches<Hole<"K">>(Keys([m1.A, m1.Ignore]))
    if (m2) {
      if (matches<keyof typeof m1.A>(m2.K)) {
        return MaybeReturn(m1.A[m2.K])
      }
      return never
    }
    return never
  }
  return never
}
/* compiles to:
 * export type Values<X extends [any, any]> = X extends [infer A, infer Ignore] ? Keys<[A, Ignore]> extends infer K ? K extends keyof A ? MaybeReturn<A[K]> : never : never : never
 */

// ✓ Ignore: verified type-identical to the original
/* @scripttype preserveParamNames */
export function Ignore(X) {
  const m1 = matches<{ [ignoreSymbol]?: Hole<"Obj">; }>(X)
  if (m1) {
    return keyof(NonNullable(m1.Obj))
  }
  return never
}
/* compiles to:
 * export type Ignore<X> = X extends { [ignoreSymbol]?: infer Obj; } ? keyof NonNullable<Obj> : never
 */

// ✓ ExtractTypes: verified type-identical to the original
/* @scripttype preserveParamNames */
export function ExtractTypes(X) {
  const m1 = matches<{ [typeSymbol]?: Hole<"_Type">; [unifySymbol]?: Hole<"_Unify">; }>(X)
  if (m1) {
    return [NonNullable(m1._Unify), Ignore(X)]
  }
  return never
}
/* compiles to:
 * export type ExtractTypes<X> = X extends { [typeSymbol]?: infer _Type; [unifySymbol]?: infer _Unify; } ? [NonNullable<_Unify>, Ignore<X>] : never
 */

// ✓ FilterIn: verified type-identical to the original
/* @scripttype preserveParamNames */
export function FilterIn(A) {
  if (matches<any>(A)) {
    if (matches<keyof typeof A>(typeSymbol)) {
      return A
    }
    return never
  }
  return never
}
/* compiles to:
 * export type FilterIn<A> = A extends any ? typeSymbol extends keyof A ? A : never : never
 */

// ✓ FilterInUnmatched: verified type-identical to the original
/* @scripttype preserveParamNames */
export function FilterInUnmatched(A, K) {
  if (matches<any>(A)) {
    if (matches<keyof typeof A>(typeSymbol)) {
      const m1 = matches<{ [unifySymbol]?: Hole<"U">; }>(A)
      if (m1) {
        if (matches<[ never ]>([Extract(keyof(NonNullable(m1.U)), K)])) {
          return A
        }
        return never
      }
      return A
    }
    return never
  }
  return never
}
/* compiles to:
 * export type FilterInUnmatched<A, K> = A extends any ? typeSymbol extends keyof A ? A extends { [unifySymbol]?: infer U; } ? [Extract<keyof NonNullable<U>, K>] extends [never] ? A : never : A : never : never
 */

// ✓ FilterOut: verified type-identical to the original
/* @scripttype preserveParamNames */
export function FilterOut(A) {
  if (matches<any>(A)) {
    if (matches<keyof typeof A>(typeSymbol)) {
      return never
    }
    return A
  }
  return never
}
/* compiles to:
 * export type FilterOut<A> = A extends any ? typeSymbol extends keyof A ? never : A : never
 */

// ✓ Unify: verified type-identical to the original
/* @scripttype preserveParamNames */
export function Unify(A) {
  const m1 = matches<Hole<"Z">>(Values(ExtractTypes(merge(FilterIn(A), { [typeSymbol]: A }))))
  if (m1) {
    return m1.Z | FilterInUnmatched(A, Keys(ExtractTypes(merge(FilterIn(A), { [typeSymbol]: A })))) | FilterOut(A)
  }
  return never
}
/* compiles to:
 * export type Unify<A> = Values<ExtractTypes<FilterIn<A> & { [typeSymbol]: A }>> extends infer Z ? Z | FilterInUnmatched<A, Keys<ExtractTypes<FilterIn<A> & { [typeSymbol]: A }>>> | FilterOut<A> : never
 */

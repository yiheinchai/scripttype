 ...
 * 7 type-level aliases: 4 covered, 2 raw, 1 typecheck-error
 *
...
// ✗ MiddlewareEntry: the ScriptType does not itself typecheck as TypeScript
//   MiddlewareEntry.st.ts(3:71) TS2348: Value of type 'MapConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function MiddlewareEntry(State = unknown, DispatchType: Dispatch<UnknownAction> = Dispatch(UnknownAction)) {
  return { middleware: Middleware(any, State, DispatchType), applied: Map(MiddlewareAPI(DispatchType, State), ReturnType(Middleware(any, State, DispatchType))) }
}
/* compiles to:

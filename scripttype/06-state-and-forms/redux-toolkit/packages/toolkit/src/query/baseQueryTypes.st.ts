...
// ✗ BaseQueryEnhancer: uses raw() — language gap, does not count as covered
...
export function BaseQueryEnhancer(AdditionalArgs = unknown, AdditionalDefinitionExtraOptions = unknown, Config = voidType()) {
  return raw('<BaseQuery extends BaseQueryFn>(baseQuery: BaseQuery, config: Config) => BaseQueryFn<BaseQueryArg<BaseQuery> & AdditionalArgs, BaseQueryResult<BaseQuery>, BaseQueryError<BaseQuery>, BaseQueryExtraOptions<BaseQuery> & AdditionalDefinitionExtraOptions, NonNullable<BaseQueryMeta<BaseQuery>>>')
}

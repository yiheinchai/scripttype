 ...
 * 26 type-level aliases: 22 covered, 2 typecheck-error, 1 compile-error, 1 raw
 *
...
type HasMatchFunction<A = any, B = any, C = any, D = any, E = any, F = any, G = any, H = any> = any
...
// ✗ AtLeastTS35: uses raw() — language gap, does not count as covered
//   gap: generic function type
/* @scripttype preserveParamNames */
export function AtLeastTS35(True, False) {
  return [True, False][IsUnknown(ReturnType(raw('<T>() => T')), 0, 1)]
}
...
// ✗ ExcludeFromTuple: the ScriptType does not itself typecheck as TypeScript
//   ExcludeFromTuple.st.ts(11:5) TS2322: Type 'any[]' is not assignable to type 'never[]'.   Type 'any' is not assignable to type 'never'.
/* @scripttype preserveParamNames */
 ...
 * type ExcludeFromTuple__loop<T, Acc extends readonly unknown[], E> = T extends [infer Head, ...infer Tail] ? ExcludeFromTuple__loop<Tail, [...Acc, ...([Head] extends [E] ? [] : [Head])], E> : Acc
 */
...
/* compiles to:
 * export type ExtractStoreExtensionsFromEnhancerTuple<EnhancerTuple extends readonly any[], Acc extends {}> = EnhancerTuple extends [infer Head, ...infer Tail] ? ExtractStoreExtensionsFromEnhancerTuple<Tail, Acc & (Head extends StoreEnhancer<infer Ext> ? IsAny<Ext, {}, Ext> : {})> : Acc
 */
...
/* compiles to:
 * export type ExtractStateExtensionsFromEnhancerTuple<EnhancerTuple extends readonly any[], Acc extends {}> = EnhancerTuple extends [infer Head, ...infer Tail] ? ExtractStateExtensionsFromEnhancerTuple<Tail, Acc & (Head extends StoreEnhancer<any, infer StateExt> ? IsAny<StateExt, {}, StateExt> : {})> : Acc
 */
...
// ✗ SafePromise: the ScriptType does not itself typecheck as TypeScript
//   SafePromise.st.ts(3:16) TS2348: Value of type 'PromiseConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function SafePromise(T) {
  return merge(Promise(T), { __linterBrands: 'SafePromise' })
}
/* compiles to:

...
export function Steps(TSnapshot: Snapshot<unknown>, TEvent: EventObject) {
  return Array(Step(TSnapshot, TEvent))
}
/* compiles to:
...
// ✗ EventExecutor: the ScriptType does not itself typecheck as TypeScript
//   EventExecutor.st.ts(3:44) TS2348: Value of type 'PromiseConstructor' is not callable. Did you mean to include 'new'?
/* @scripttype preserveParamNames */
export function EventExecutor(TSnapshot: Snapshot<unknown>, TEvent: EventObject) {
  return fnType([Step(TSnapshot, TEvent)], Promise(any) | voidType())
}
/* compiles to:
...
export function PathGenerator(TSnapshot: Snapshot<unknown>, TEvent: EventObject, TInput) {
  return fnType([ActorLogic(TSnapshot, TEvent, TInput), TraversalOptions(TSnapshot, TEvent, TInput)], Array(StatePath(TSnapshot, TEvent)))
}
/* compiles to:

/**
 * ScriptType for 06-state-and-forms/xstate/packages/xstate-store-solid/src/index.ts
 *
...
export function AtomStateFromDefinition(TDefinition: AtomDefinition) {
  if (matches<AnyAtomConfig>(TDefinition)) {
    return asReadonly([Accessor(ValueFromAtomConfig(TDefinition)), ReturnType(TDefinition['createAtom'])])
  }
  const m1 = matches<BaseAtom<Hole<"TValue">>>(TDefinition)
  if (m1) {
    return asReadonly([Accessor(m1.TValue), TDefinition])
  }
  return never
}
/* compiles to:
 * export type AtomStateFromDefinition<TDefinition extends AtomDefinition> = TDefinition extends AnyAtomConfig ? readonly [Accessor<ValueFromAtomConfig<TDefinition>>, ReturnType<TDefinition['createAtom']>] : TDefinition extends BaseAtom<infer TValue> ? readonly [Accessor<TValue>, TDefinition] : never
 */
